...
// =======================================
function checkNumber_if(num) { // num = 数字の入力
    // 【説明1】
    // このif文の関数の中には「5より大きいかどうか判定する」というルールが
    // 最初から固定で書かれている

    // 🚩補足：
    // 下の呼び出し例で checkNumber_if(10) と実行すると、
    // 引数 num には 10 が入る。
    // だから今回は「10 > 5」を判定していることになる。
    if (num > 5) {
...
// =======================================
// 🚩STEP2: コールバック版関数の定義（処理を差し替え可能）
// =======================================
function checkNumber_cb(num, callback) { // num = 数字, callback = 外から渡す処理
    // 【説明2】
    // このコールバック関数自体は「判定ルール」を持っていない。
    // 代わりに、外から渡された callback 関数を呼び出して
    // num を渡す仕組みになっている。

    // 🚩補足：
    // たとえば下の呼び出し例で checkNumber_cb(10, biggerOrSmaller) とすると、
    // num には 10 が入り、callback には biggerOrSmaller が入る。
    // つまり「biggerOrSmaller(10)」が実行されることになる。
    callback(num);
}
...
// =======================================
// 🚩STEP3: コールバック関数の具体例を用意
// =======================================

// 【例1】if文と同じ判定をするコールバック関数
function biggerOrSmaller(n) {
    // n は数字（ここでは後で 10 や 3 が入る）

    // 🚩補足：
    // 例えば呼び出し例で checkNumber_cb(10, biggerOrSmaller) とした場合、
    // n には 10 が渡される。
    // だから「10 > 5」を判定することになる。
    if (n > 5) {
        console.log("コールバック → 大きい（今回は n が 5 より大きいから）");
    } else {
...
// 【例2】入力された数字を2倍して表示するコールバック関数
function doubleNumber(n) {
    // 🚩補足：
    // 呼び出し例で checkNumber_cb(5, doubleNumber) とすると、
    // n には 5 が渡される。
    // だから「5 を 2倍した 10」が出力される。
    console.log("コールバック → 2倍 =", n * 2);
}
...
// - コールバック版は「判定」や「2倍」など外から自由に差し替えられる
// =======================================

console.log("=== 入力値 10 の場合 ===");
checkNumber_if(10);                 // if文版（固定ルール）
checkNumber_cb(10, biggerOrSmaller); // コールバック版（判定処理）

console.log("=== 入力値 3 の場合 ===");
checkNumber_if(3);                  // if文版（固定ルール）
checkNumber_cb(3, biggerOrSmaller);  // コールバック版（判定処理）

console.log("=== 入力値 5 の場合 ===");
checkNumber_if(5);                  // if文版（固定ルール）
checkNumber_cb(5, doubleNumber);     // コールバック版（別の処理）
...
// =======================================
// 📝 違いまとめ
// - if文版：関数の中にルールが書き込まれているため、動きが固定
// - コールバック版：ルールを外から渡せるため、同じ骨組みでも動きを差し替え可能
// =======================================
