...
const counter = {
  ...
  increment: function () {
    //メソッド名increment。{}内は処理。for文で繰り返す。
    for (let i = 0; i < 1; i++) {
      // 変数i。初期値:変数iは最初は0、条件式、増減式:変数iを1ずつ増やす。1回ループ
      this.value++; // 値を1増やす
    }
...
///undefindがあらわる理由
function hello3() {//関数hello3
    console.log("hello3はこんにちはがでますか？");//←処理内容にreturn戻り値がない
  }
  
  let result3 = hello3();//呼び出し元
console.log(result3); // → これは undefined と出る
  


///returnであらわる理由
function hello4() {//関数hello4
    return "returnを使うとhello4はこんにちはがでます！";//←処理内容にreturn戻り値がある
  }
  
  let result4 = hello4();//呼び出し元。
  console.log(result4); // → これは "returnを使うとhello" が表示されます。
