...
// 関数名の後ろに () をつけると実行され、戻り値が resultValue に入る
// ここで resultValue に入っているのは「数値 7777」であり関数ではない
// そのため、もし resultValue(); とすると
// 「数値を関数として呼び出そうとしている」ことになりエラーになる
// 関数として呼び出せるのは hyperDrive() のように関数名そのもの

console.log(`波括弧$resultValue で 定数hyperDrive の戻り値 7777 を参照して出力する。→ ${resultValue}`);

// --------------------------------------------------
// 関数名そのものを直接呼び出す例
// --------------------------------------------------
console.log(`波括弧$hyperdrive で 関数 hyperDrive() を直接呼び出した。 → ${hyperDrive()}`);
console.log(`波括弧${resultValue} で 定数 hyperDrive の戻り値 7777 を参照する。→ ${resultValue}`);
