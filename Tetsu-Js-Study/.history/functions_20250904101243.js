...
function skeletonFunction(num, actionFunction) { 
    // 第一引数 = num 数字,  第二引数 = actionFunction コールバック引数 外から渡す処理関数

    // 【説明】
    // 骨組み関数 skeletonFunction は 呼び出し側から「数字を受け取って誰かに渡す」役だけをする。
    // 具体的に「どう判定するか」「どう計算するか」は知らない。
    // その部分は外から来る コールバック引数 actionFunction に任せる。
    
    // 例：呼び出し側で skeletonFunction(10, judgeFunction) とすると
    // num（引数の左側） には 10、actionFunction（引数の右側） には judgeFunction が入る。
    // つまり 骨組み関数 skeletonFunction 内で書かれている コールバック引数 actionFunction(num) は、
    // 第一引数 num = 10 、第二引数 actionFunction = judgeFunction によって
    // → judgeFunction(10) に置き換わる。
    // だから実際には コールバック関数  judgeFunction(10) が実行される。
