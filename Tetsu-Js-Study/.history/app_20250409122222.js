...
const pikachuget = () => {
    if(Math.random() <= 0.2){//20%以下かどうか。Math.random()は1以下の数字「0～0.999999…」をランダムに生成する。
        return "「ピカチュウをゲットした！」";//当てはまる場合（20%）return
    }else {//当てはまらない場合（80%）
...
document.querySelector("#button33").addEventListener("click", function () {//HTMLのid=button33を取得する。
    console.log(pikachuget() + "← ピカチュウゲット確率ゲームの結果を表示する");//イベント 確率ゲーム（変数pikachuget）
});
