...
const pikachu2 = {
  ...
  levelUp: function () {
      for (let i = 0; i < 1; i++) {//真ん中の条件式(i < 1)のために変数iは1回だけループする。
          this.value2++;//pikachu2.levelUp(オブジェクトのプロパティ)がインクレメント++で1ずつ増加する。
    ...
    powerUp: function(){
        if (this.value2 >= 20) {//もしthis.value2(pikachu2オブジェクトのプロパティvalue2)の値が20以上の場合
            this.skills.push("スパーク");//this.skills()にpushプロパティを追加する。
        }
        console.log(this.skills[3]);//this.skillsを表示する。
        console.log(this.types[0]);
    },
};

//インクレメント++はオブジェクトpikachu2の中で記憶され初期値value2を増やしていく。
