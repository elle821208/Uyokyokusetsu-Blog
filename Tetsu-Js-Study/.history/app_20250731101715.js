...
// 🔍 "2"（文字の2）を「数字の2」に変えるには parseInt を使います。
//      parseInt("2") と書くと、本物の数字の 2 に変身します。
//      だから計算に使ったり、数字として足したりできます！

console.log("2" + " ←これは文字の2です（数字じゃないよ）");
// 📝 この「2」は見た目は数字っぽいけど、文字（文字列）です。
//      つまり「おはよう」とか「りんご」と同じ仲間です。
//      計算はできません。ただの文字です。

console.log(console);
// 💡 console（コンソール）は、画面にメッセージを表示するための道具です。
//    このコードでは console.log を使って、いろいろなことを画面に表示しています。
//    これを書いて「中身ってどうなってるのかな？」と調べることもできます。

console.log(Math);
// 📐 Math（マス）は「数学の道具セット」です。
//    たとえば「足し算、引き算、まるめる、さいころの目を作る（乱数）」など、
//    数に関する便利なことをしてくれます。
//    これを書いて中身を見てみると、「Math ってこんなことできるんだ！」と発見できるよ。
