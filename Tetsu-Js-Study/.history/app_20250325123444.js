...
if (isLogin === false) {
  ...
  alert("isLoginさんはfalseなのでログイン1してください"); //左の警告文を出す
}
...
if (!isLogin2) {///条件式:もしisLogin2がtrueと等しくない場合は(!で等しくないの意味)
    alert("isLogin2さんはログイン2してください");//左の警告文を出す
} else {//上記のif文が満たされない場合は
  alert("isLogin2さんはログイン2が成功中"); //左の警告文を出す
}

//elseif条件式 member社員 admin管理 owner社長 の三つの権限を持たせる <!--1:15:00～-->
const userType = "admin管理";//定数を宣言する。定数userTypeの値は"member"
if (userType === "member社員") {//もし定数userTypeの値が"member"の場合
    alert("定数userTypeが「member社員」でないためアクセス3できません");//左を実行する。
} else if (userType === "admin管理") {////もし定数userTypeの値が"admin"の場合
    alert("定数userTypeが「admin管理」なので今から10分だけアクセス3できます");//左を実行する。
} else {///member社員 admin管理でもない場合（owner社長の場合）
    console.log("owner社長はアクセス3成功");
}

//Q.「myMoneyが500以上ならisShippingでtrueを返す」<!--1:17:30～-->
const myMoney = 1000;//変数を宣言: myMoneyの値は1000
let isShipping = false;//変数を宣言: isShippingの値はfalse
if (myMoney >= 500) {//条件式: もしmyMoneyが500以上なら
    let isShipping = false;//変数isShipping false を 
    isShipping = true;//true に再代入する処理を行う。
}
