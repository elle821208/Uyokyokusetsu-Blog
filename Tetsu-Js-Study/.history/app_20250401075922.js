...
window.onload = function () {
    ...
    const hornDrill = (Math.floor(Math.random()+0.3));// ※※30%で数字1を出す。(1は当たり）、70%で数字0を出す。（0はハズレ）

    if (hornDrill === 0) {//かつで30%なら
        alert("成功hornDrill1です。");
        //成功したら戻り値theirHpの値をそのまま返す
        //return
    } else if (hornDrill === 1) {
      alert("成功hornDrill2です。");
    } else if (hornDrill === 2) {
        alert("失敗hornDrill1です。");
        //失敗なら0を返す。
    } else if (hornDrill === 3) {
      alert("失敗hornDrill2です。");
    } else if (hornDrill === 4) {
      alert("失敗hornDrill3です.....");
    }
  }
  ...
  //30％の確率で「当たり」を設定する。
// ※※30%で数字1を出す。(1は当たり）、70%で数字0を出す。（0はハズレ）
const ooo =(Math.floor(Math.random()+0.3));
console.log(ooo);
