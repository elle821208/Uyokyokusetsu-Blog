...
// function で関数を定義し、() 内に引数を設定する
function introduce(name, job) {
    // name, job という引数を受け取り、関数の中で使う
    console.log("ここが呼び出し元。上記の関数 introduce に、引数の値が入り、ここで呼び出される。→ 私は" + name + "です。");
    console.log(job + "です。");
}

// 関数の呼び出し方
introduce("哲也。※引数1で上記のnameに渡る", "プログラマー。※引数2で上記のjobに渡る");

// 👉 上の実行結果：
// 私は哲也。※引数1で上記のnameに渡るです。
// プログラマー。※引数2で上記のjobに渡るです。

// =======================================
// 2. 関数式（無名関数）の定義
// =======================================

// 🔸関数式とは：関数を変数（定数）に代入する書き方
// 🔸無名関数とは：名前のない関数のこと
const introduce2 = function (name, job) {
    // 関数内では引数 name と job を使って処理する
    console.log("ここが呼び出し元。下記のintroduce2('tetsuya', 'コーダー')で関数式を呼び出し、引数の値が上記の function(name, job) に入る。→ 私は" + name + "です。");
    console.log(job + "です。");
};
...
// ポイントまとめ
// =======================================
// 1. function を使うと関数を定義できる
// 2. () 内の値は引数として関数に渡される
// 3. 関数は呼び出すまで処理は実行されない
// 4. 関数式（無名関数）とは、名前のない関数を定数に代入した書き方
// 5. 関数式も定数名 + () で呼び出すことができる
// 6. console.log() で出力することで、関数内の処理結果を確認できる
