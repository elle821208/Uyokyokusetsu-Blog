...
// 2. 現在の月を取得（currentMonth に代入する準備処理）
// 💡 開発者視点: ここで現在の月を計算して「変数 currentMonth」に入れておく
// 💡 ユーザー視点: まだ「動いた」と感じる処理はない（裏で準備してるだけ）
const currentMonth = new Date().getMonth() + 1; 
console.log(`👉 現在の月 (currentMonth): ${currentMonth} （今回の値: 8）`);

// 3. 関数を呼び出して季節を取得
// 👀 ユーザー視点: ここから「動作が始まった」と感じる
// （ここで初めて関数が実行され、季節が判定される）
const season = getSeason(currentMonth);
console.log(`👉 関数 getSeason に currentMonth (${currentMonth}）を渡した結果: ${season} （今回の値: 夏）`);

// 4. 結果をコンソールに表示
// 👀 ユーザー視点: 季節が表示されるのを確認できる部分
...
// =======================================
// 【開発者視点の流れ】
// 1. getSeason(month) を定義（準備）
// 2. currentMonth に「今の月（1～12）」を代入 → 今回は 8
// 3. getSeason(currentMonth) を実行 → getSeason(8)
// 4. 引数 month=8 が関数に渡され、条件分岐で「夏」と判定 → 戻り値 "夏"
// 5. 戻り値が season に代入され、結果を表示

// 【ユーザー視点の動作開始】
// 👉 3. getSeason(currentMonth) を呼び出すところから
// 👉 そこから季節が判定されて「夏」と表示されるのを見て動作したと感じる
