...
///<!--2:02:19～-->
//querySelector
console.log(" ↓ querySelectorで一つのid #foofooを一つの変数として取得した。htmlにおいてidは1ページに一つしか存在できない。");
console.log(document.querySelector("#foofoo"));//idは#をつける。一つの変数としてカウントされる。htmlにおいてidは1ページに一つしか存在できない。
console.log(" ↓ querySelectorAllで複数あるのclass .barbarを配列としてまとめて取得した。htmlにおいてclassは1ページに複数存在できる。");
console.log(document.querySelectorAll(".barbar"));//classは.をつける。classを全て取得する。配列になる。
console.log(" ↓ querySelectorAllで複数のclass .barbarの[]で配列の一つ目を指定し、innerHTMLで編集する。");
console.log(document.querySelectorAll(".barbar")[0].innerHTML = "<div>.barbarクラスの一つ目の配列をinnerHTMLで編集する</div>"  );//classは.をつける。classを全て取得する。配列になる。
