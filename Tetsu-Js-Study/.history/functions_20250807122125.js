...
// ▼ メイン関数（a）：コールバック関数を引数として受け取る
function a(b) { 
  // b = コールバック関数を受け取る「コールバック引数」（通称：受け取り役・呼び出し口）
  console.log("Hi,Cat!"); // メイン処理（司会のあいさつ）
  b(); // ← b の中身は実際に渡された関数（c）＝「追加処理（コールバック関数）」
}

// ▼ 追加関数（c）：実際に渡す関数（実行される処理）
// 通称：追加処理関数、コールバック関数の「実体」
// 用途：あとから呼ばれる関数（bに渡されて実行される）
function c() {
  console.log("Hi,Bat!"); // 追加処理（ゲストのあいさつなど）
}

// ▼ 実行：関数aにcを渡して実行する
// 通称：呼び出し元／エントリーポイント
// a(c) → b = c として渡される → 関数aの中で b() → 実体c() が呼ばれる
a(c);
// ▼ メイン関数 a（司会役）
//    引数 b にコールバック関数（＝追加関数）を受け取る
function a(b) {
  console.log("メイン関数 a の処理 →" + "Hi,Cat!"); // (3) メインのあいさつを表示
...
function c() {
  console.log("コールバック関数 c の処理 →" + "Hi,Bat!"); // (6) 追加のあいさつを表示
}
