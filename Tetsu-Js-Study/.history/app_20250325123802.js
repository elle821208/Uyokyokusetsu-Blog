...
const userType = "hohohoh";//定数を宣言する。定数userTypeの値は"member"
if (userType === "member") {//もし定数userTypeの値が"member"の場合
    alert("定数userTypeがただの「member」なのでアクセス3できません");//左を実行する。
} else if (userType === "admin") {////もし定数userTypeの値が"admin"の場合
    alert("定数userTypeがただの「admin」なので今から10分だけアクセス3できます");//左を実行する。
} else {///member社員 admin管理でもない場合（owner社長の場合）
    alert("ownerは自由にアクセス3成功できます");
}
