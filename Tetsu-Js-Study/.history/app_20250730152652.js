...
//グローバル変数
// 🌏 =======================================
// 🌏 (1) グローバル変数を定義（グローバルスコープ）
// 🌏 =======================================
// 「グローバル変数」とは、プログラム全体（どこからでも）使える変数。
// 例えるなら「学校全体で使うルール」のようなもの。

let number = 1; // 🌏 (1-1) グローバル変数 number を作り、「1」を入れる。
// この変数は、プログラムのどこからでも使える。

// 🏠 =======================================
// 🏠 (2) 関数 showNumber を作る（ローカルスコープの世界）
// 🏠 =======================================
// 「関数」とは、やりたいことをまとめた「手順書」や「レシピ」のようなもの。
// この関数を使うと、決まった動作を1回の命令でまとめて実行できる。

function showNumber() {
  // 🏠 (2-1) ここから関数 showNumber の中の世界（ローカルスコープ）。
  // 関数の中で作る変数は「ローカル変数」と呼ばれ、
  // この関数の中だけでしか使えない特別な変数になる。

  const number = 2; // 🏠 (2-2) 関数の中だけで使える「ローカル変数 number 」を作り、「2」を入れる。
  // この number は 🌏 (1-1) のグローバル変数と「同じ名前」だけど、「別物」です。

  console.log("🏠 関数の中でconsoleで呼び出す。 ローカル変数number の値 2。 ※後で関数の外で関数showNumber();を実行しないとここは表示されない。 → " + number); 
  // 🏠 (2-3) 結果：2 が表示される。
  // なぜなら、関数の中ではローカルな number（＝2）を使うから。
  // ⚠️ ただし、関数を「実行」しないとこの行の処理は動かない。
}
// 🏠 (2-4) ここまでが関数 showNumber の中の世界。
// ✅ この時点では、関数は「作られただけ」でまだ動いていない。

// 👇 =======================================
// 👇 (3) 関数を実行する（関数の外で）
// 👇 =======================================
// ↓ここは関数の外の世界＝グローバルスコープ（🌏）。
// 「関数の外」＝プログラム全体のメインの場所。
// 関数の外から関数を呼び出すことで、
// 必要なタイミングで関数の中の命令を実行できるようになります。

showNumber(); 
// 👇 (3-1) 上で定義した関数をここで「実行」する。
// すると、関数の中で定義された number（＝2）が表示される。
// ※ 関数の外で実行するのは、関数があくまで「使われる道具」だから。
// 例えば、「ごはんを作るレシピ」は道具であり、実際に料理するのはレシピを使う人（＝関数の外）です。

// 🌏 =======================================
// 🌏 (4) グローバル変数の値を表示（関数の外）
// 🌏 =======================================
console.log("🌏 関数の外でconsoleで呼び出す。 グローバル変数number の値 1。 → " + number); 
// 🌏 (4-1) 結果：1 が表示される。
// なぜなら、関数の外では (1-1) のグローバル変数 number（＝1）を使うから。
// 関数の中で作った number（＝2）は、関数の外では使えない。
...
console.log(pikachu.name + "←オブジェクトpikachuのname ピカチュウ");
console.log(pikachu.level + "←オブジェクトpikachuのlevel 18");
console.log(pikachu.skills[0] + "←オブジェクトpikachuの配列skillsの0番目 電光石火");
...
const pikachu2 = {
  // (1-1) オブジェクト pikachu2 を作成。
  //       これはポケモン「ピカチュウ」を表す情報のまとまり。

  name: "ピカチュウ",
  // (1-2) プロパティ name。値は "ピカチュウ"。
  //       キャラクターの名前を表す。

  level: 18,
  // (1-3) プロパティ level。値は 18。
  //       現在のレベルを数値で表している。

  types: ["でんき"],
  // (1-4) プロパティ types。値は配列 ["でんき"]。
  //       このポケモンのタイプ（属性）を示す。

  skills: ["10万ボルト", "でんこうせっか", "たいあたり"],
  // (1-5) プロパティ skills。技を格納した配列。
  //       今のところ 3 つの技がある。
  //       ※あとで条件を満たすと 4 番目の技 "スパーク" が追加される予定。

  levelUp: function () {
    // (1-6) メソッド levelUp を定義。function キーワードで定義した通常の関数。
    //       ピカチュウのレベルを 1 上げる動作をする。

    this.level++;
    // (1-7) インクリメント演算子（++）を使ってレベルを 1 つ増やす。
    //       this.level は pikachu2.level を指す。
    //       実行するたびにレベルが 18 → 19 → 20 … と増えていく。

    // 🧠【インクリメント（++）とは】
    // - 変数に対して「1 だけ増やす」操作。
    // - 一度上がったレベルは次の実行でも保持される（状態が記録される）。

    if (this.level >= 20) {
      // (1-8) 条件分岐。レベルが 20 以上になったとき、
      //       新しい技「スパーク」を習得させる。

      this.skills.push("スパーク");
      // (1-9) 配列 skills に新しい要素 "スパーク" を追加する。
      //       push() は配列の末尾に要素を追加するメソッド。
    }

    // ✅【this のポイント】
    // - this.level や this.skills は、オブジェクト pikachu2 自身を参照している。
    // - つまり「このオブジェクトのプロパティを操作する」ことになる。
    // - 再利用可能性が高く、複数のキャラに応用しやすい。
  },
};
...
console.log("呼び出し元console。オブジェクトPikachu2の初期のレベル18 レベル18のスキル。→:", pikachu2.level, pikachu2.skills);
// (2-1) 初期状態を表示。レベルは 18、技は 3 つ。

pikachu2.levelUp();
// (2-2) メソッド levelUp を 1 回実行。
//       → レベルが 19 になる。スパークはまだ追加されない。

console.log("呼び出し元console。↑上の行で関数メソッドlevelUpを一回実行している。オブジェクトPikachu2のレベル19 レベル19のスキル。→:", pikachu2.level, pikachu2.skills);
// (2-3) レベル 19 を表示。まだ "スパーク" は習得していない。

pikachu2.levelUp();
// (2-4) メソッド levelUp をもう一度実行。
//       → レベルが 20 になり、条件を満たすので "スパーク" を追加。

console.log("呼び出し元console。↑上の行で関数メソッドlevelUpを一回実行している。関数メソッドlevelUpにthis.level++;インクレメントとif文を組んでレベル20になったらスキル スパークをついかする。オブジェクトPikachu2のレベル20 レベル20でスキルにスパークが追加される。→:", pikachu2.level, pikachu2.skills);
// (2-5) レベル 20、スキルが 4 つになっている状態を表示。

console.log(pikachu2.level, pikachu2.skills);
// (2-6) 現在の状態（レベルと技）を最終確認。

// 🧠【まとめ】
// - オブジェクトの中の数値プロパティは、関数で操作して変化させられる。
// - メソッド内で this を使うことで、オブジェクト自身のプロパティを動的に参照・更新できる。
// - 条件付きで配列に要素を追加する処理（if + push）も合わせて学べる良い例。
