...
console.log(snsUser.id + "←snsUser.id で上記の設定した情報（constで定義したオブジェクト snsUser {}内のプロパティ id: プロパティの値 1）を呼び出す。"); //オブジェクト名.プロパティ名で値を取り出す。
console.log(snsUser.like() + "←snsUser.like() で上記のプロパティlike関数の値「まいどおおきに」だけを取り出す。"); //オブジェクト名.プロパティ名で値を取り出す。
console.log(snsUser.followers[0] + "←snsUser.followers[0] で上記のプロパティpost関数の配列followersの]0]番目yamadaを出力"); //配列followersの0番目を出力
console.log(snsUser.settings.darkMode + "snsUser.settings.darkMode で 上記のプロパティsettingsの中のプロパティdarkModeの値falseを出力");
