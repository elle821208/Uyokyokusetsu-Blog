...
function checkNumber_if(num) { // num = 数字の入力
    ...
    // 最初から固定で書かれている
    if (num > 5) {
        console.log("if文 → 大きい（今回は呼び出し側が 入力値が10で checkNumber_if(10) なので 引数numには10が入る）");// ← ※※※下記の🚩STEP4: 呼び出し例（比較テスト）で入力値10で呼び出される！
    } else {
        console.log("if文 → 小さい");
    }
}
...
function checkNumber_cb(num, callback) { // num = 数字, callback = 外から渡す処理
    // 【説明2】
    // ここでは「具体的に何をするか」は決めずに
    // 外から渡された callback を呼び出すだけにしている
    callback(num); // callback に num を渡して実行
}
...
function biggerOrSmaller(n) {
    ...
    // もし数字が 5 より大きかったら「大きい」と言う
    if (n > 5) {
        // 🚩ポイント：
        // この console.log が動くのは、
        // 後で呼び出し側で「checkNumber_cb(10, biggerOrSmaller)」と呼ばれるから。
        // そのとき n に 10 が入って「10 > 5」が正しいので、
        // 「コールバック → 大きい」がコンソールに出る！
        console.log("コールバック → 大きい"); 
    } else {
        // もし数字が 5 以下なら「小さい」と出る
