...
console.log(document.querySelector("#foofoo").innerHTML = "<div>idをひとつの変数として取得した。htmlにおいてidは1ページに一つしか存在できない。</div>");//idは#をつける。一つの変数としてカウントされる。htmlにおいてidは1ページに一つしか存在できない。
console.log(document.querySelectorAll(".barbar")[0].innerHTML = "<div>querySelectorAllで4つの.barbarクラスを配列としてまとめた。これは[0]番目。</div>");
console.log(document.querySelectorAll(".barbar")[1].innerHTML = "<div>配列の中身を[番号]で指定し、innerHTMLで編集する。これは[1]番目。※※querySelectorAll（idもクラスも取得可）で取得した。</div>");
//単純なクラス名での選択の場合は`getElementByClassName`が便利
console.log(document.getElementById("foofoo") );//id。一つの変数としてカウントされる。
console.log(document.getElementsByClassName("barbar")[2].innerHTML = "<div>同様に配列を編集した。これは[2]番目。※※getElementsByClassName（クラスのみ有効）で取得した。</div>");//classを全て取得する。配列になる。

///<!--2:04:30～-->
const $zoozoo = document.querySelectorAll(".barbar");//変数に4つのクラス（querySelectorAll(".barbar")）を格納しておく。

$zoozoo[3].innerHTML = "<div>変数$zoozooにquerySelectorAll（.barbarの4つのクラス）を格納した後、この変数をinnerHTMLで編集する。これは[4]番目。</div > ";