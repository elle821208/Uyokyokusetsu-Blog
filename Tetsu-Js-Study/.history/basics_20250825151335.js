...
console.log(`🎯 ↑↑↑ 今回の初期設定 → hasTicket = ${hasTicket}, isAdult = ${isAdult}`);

// 「チケットを持っていて、なおかつ大人なら入場できる」
if (hasTicket && isAdult) { // 両方 true なら実行
  console.log("🎫 ↑↑↑ チケットもあって大人なので入場できます！ ✅");
  console.log(`📝 ↑↑↑ 条件判定:今回の初期設定は hasTicket = ${hasTicket}, isAdult = ${isAdult}  である。 → よって両方 true の場合なので、このメッセージが表示される`);
} else {
  console.log("🚪 ↑↑↑ チケットがないか、子供なので入場できません… ❌");
  console.log(`📝 ↑↑↑ 条件判定: 今回の初期設定は hasTicket = ${hasTicket}, isAdult = ${isAdult}  である。 → よってどちらかが false 場合なので、このメッセージが表示される`);
}
...
console.log(`🎯 ↑↑↑ 今回の初期設定 → ateMeal = ${ateMeal}, ateSnack = ${ateSnack}`);

// 「ご飯を食べた または お菓子を食べた ならお腹は満足」
if (ateMeal || ateSnack) { // どちらかが true なら実行
  console.log("🍽 ↑↑↑ ご飯かお菓子を食べたのでお腹は大丈夫！ 😋 ✅");
  console.log(`📝 ↑↑↑ 条件判定: 今回の初期設定は ateMeal = ${ateMeal}, ateSnack = ${ateSnack} である。 → よって「どちらかが true」なので、このメッセージが表示される`);
} else {
  console.log("🍴 ↑↑↑ 何も食べてないのでお腹が空いた… 🥺 ❌");
  console.log(`📝 ↑↑↑ 条件判定: 今回の初期設定は ateMeal = ${ateMeal}, ateSnack = ${ateSnack} である。 → よって「両方 false」なので、このメッセージが表示される`);
}
