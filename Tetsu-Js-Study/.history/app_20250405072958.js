...
const myMoney = 1000; //その１定数を宣言: myMoneyの値は1000を代入する
let isShipping = false; //変数を宣言: isShippingの値はfalseを代入しておく。
//上記で定義した定数myMoneyと定数の値（今回は"1000"）を下記のif文で判定する。
if (myMoney >= 500) {
    //条件式: もし定数myMoneyが500以上なら
    //{}内の処理を行う。
    //変数名isShippingの頭にletをつけないことで変数isShippingに値trueを再代入する処理を行う。
  isShipping = true; 
}
