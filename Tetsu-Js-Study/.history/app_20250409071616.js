...
///<!--2:08:03～-->
////空っぽのHTMLクラス(.timeline)にappendChildで情報を持った変数($)を入れ込む。
...
const $timeline = document.querySelectorAll(".timeline")[0]; //querySelectorAllでHTMLに記述した.timeline(引数)クラスの[]番目を取得する。
...
$pikachu22.innerText = "ピカチュウ！";
//空のid=outputを取得し、DOM操作で$pikachu22を組み込んで、タグ、クラス、クラス名などを追加する。
const $output = document.querySelector("output"); //HTMLのid="output"を取得
//id=button22を取得しイベントを付与する。イベント内容はクリックしたらHTMLに記載された$outputにDOM操作で$pikachu22組み込まれ、"ピカチュウ！"が表示される
document.querySelector("#button22").addEventListener("click", function () {
  // ↓↓↓ addEventlistenerのイベント内容
