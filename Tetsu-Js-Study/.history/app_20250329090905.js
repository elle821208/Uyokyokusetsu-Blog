...
function introduce(name, job) {//関数introduce 引数(関数に与える情報)name、job
    console.log(`私は${name}です。`)//関数の処理内容。
    console.log(`${job}です。`)//関数の処理内容。
}

introduce('ta1fukumoto', 'エンジニア');//上記で設定した関数introduceを実行する。あらかじめ引数の値も設定しておく。
//関数introduceを実行すると設定した引数の値('ta1fukumoto', 'エンジニア')が関数のname、jobに値が渡される。
//下記が表示される。
//私はta1fukumotoです。
//エンジニアです。
