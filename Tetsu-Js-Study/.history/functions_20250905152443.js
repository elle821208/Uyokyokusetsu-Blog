...
// ======= セクション開始：設定と基本の考え方 =======
console.log("🚩【セクション0】背景色と季節を判定するプログラム（引数＆戻り値の基本例）");
console.log("   🕒 new Date() = 現在日時");
console.log("   ⏰ getHours() = 時間だけ取り出す");
console.log("   📅 getMonth()+1 = 月だけ取り出す\n");
...
function getDarkModeStatus(time) {
    // 5️⃣ 関数内で if 文による判定（time が 20 より大きいかどうか）
    if (time > 20) {
...
// 2️⃣ new Date() で現在の時刻を取得し、.getHours() で時間を取り出す
// 3️⃣ getDarkModeStatus(...) を呼び出す
const mode = getDarkModeStatus(new Date().getHours()); 

// 6️⃣ 戻り値 "dark" または "light" が変数 mode に格納される

// 7️⃣ 変数 mode を利用して背景色を変更
if (mode === "dark") {
...
// 8️⃣ 結果を出力
console.log(`現在のモード: ${mode} （今回の設定の2025年8月28日より dark）`);
...
function getSeason(month) {
    // 5️⃣ 関数内で if 文による判定（月の範囲ごとに季節を判定）
    if (month >= 3 && month <= 5) {
...
// 2️⃣ new Date() で現在の月を取得し、.getMonth() で月を取り出し、+1 で整形
const currentMonth = new Date().getMonth() + 1;

// 3️⃣ getSeason(currentMonth) を呼び出す
const season = getSeason(currentMonth); 

// 6️⃣ 戻り値 "春" / "夏" / "秋" / "冬" が変数 season に格納される

// 7️⃣ 変数 season を利用して結果を出力
console.log(`現在の月は ${currentMonth} 月です （今回の設定の2025年8月28日より 8）`);
console.log(`🌟 なので、季節は ${season} です！ （今回の設定の2025年8月28日より 夏）`);
...
// ======= セクション開始：数字を渡すパターン =======
console.log("🚩【セクション0】数値を渡す例（コールバックなし・基本形）");
console.log("    f = 骨組み関数, g = 引数（数値）, i = 渡す値\n");
...
function f(g) {
    // 【説明1】
    // メイン処理：必ず実行される基本の動作（骨組み部分）
    console.log("【1】メイン処理（f の骨組み部分）：必ず実行される固定の動作");

    // 【説明2】
    // 骨組み関数の中でも「追加処理」を書ける
    // これは必ず実行される（差し替えできない固定処理）
    console.log("【2】骨組み関数の追加処理（f 内）：毎回必ず実行される");

    // 【説明3】
    // 今回はコールバック関数が無いので、値をそのまま表示する
    console.log("👉 今回はコールバック関数が無いので、値をそのまま表示するよ！");
    console.log("   渡された値 g =", g);
}
...
// =======================================
// 🚩 処理の流れまとめ（矢印付き・数字を渡す版）
// =======================================
//
// 呼び出し: f(i)  🚩STEP1
//        ↓
// g = i   ← f(i) = f(g) なので g は i の値 42 を指す  🚩STEP2
//        ↓
// 【1️⃣】メイン処理 f が実行（固定の骨組み部分）
//        ↓
// 関数は渡されていないので b() のような呼び出しはなく、単に g の値を表示
// 👉 実際に表示される値 g = 42  🚩STEP3
//        ↓
// 【2️⃣】骨組み関数 f の追加処理（固定で必ず実行される）
//        ↓
// 完了！ 🎉
...
// if文 vs コールバック関数 解説（統合版）
// =======================================
// 🟢 このコードの目的
// - if文版は処理が関数の中で固定されている
// - コールバック版は処理を外から差し替え可能
// → この違いを理解する
// =======================================
...
function fixedJudgeFunction(num) { // num = 数字の入力
    // 【説明】
    // この関数は「5より大きいかどうか判定する」ルールが最初から固定で書かれている。
    // 呼び出し側でルールを差し替えることはできない。
    if (num > 5) {
        console.log("if文 → 大きい！（今回は num が 5 より大きいから）");
    } else {
        console.log("if文 → 小さい！（今回は num が 5 以下だから）");
    }
}
...
function skeletonFunction(num, actionFunction) {
    // 【説明】
    // 骨組み関数 skeletonFunction は「数字を受け取って、渡された処理を呼び出すだけ」。
    // 「判定するのか」「計算するのか」は知らない。外から渡された関数に任せる。
    actionFunction(num);
}
...
// =======================================
// 呼び出し側で、どのコールバック関数を
// 骨組み関数 skeletonFunction の第2引数に渡すかによって、処理内容が変わる。
...
function judgeFunction(n) {
    if (n > 5) {
        ...
        console.log("判定関数 → 小さい！（今回は n が 5 以下だから） ← 判定系コールバック関数より");
    }
}
...
fixedJudgeFunction(10);              // 固定処理
skeletonFunction(10, judgeFunction); // 判定系コールバック関数を適用

console.log("=== 入力値 3 の場合 ===");
fixedJudgeFunction(3);
skeletonFunction(3, judgeFunction);

console.log("=== 入力値 5 の場合 ===");
fixedJudgeFunction(5);
skeletonFunction(5, doubleFunction); // ここだけ「計算系コールバック関数」を適用


// =======================================
// 🚩STEP5: 違いまとめ
// =======================================
// - if文版（fixedJudgeFunction）
//   → 処理内容が関数内に固定されていて、差し替えはできない
//
// - コールバック版（skeletonFunction + コールバック関数）
//   → 外から渡す関数を変えるだけで、判定処理や計算処理など自由に差し替え可能
// =======================================
...
//     → 実際には judgeFunction(10) が呼び出される
//
// 4️⃣ 結果
//     判定系コールバック関数 judgeFunction が処理を実行する
// =======================================
