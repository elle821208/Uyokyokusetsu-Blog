...
function greet(cb) {
    console.log(" ②メイン処理 → Hi,Amy!。"
        + "この呼び出し元とメイン処理により、この時点で greet(showName)=  greet(cb) よりshowName = cb となる。"
        + "③下記でコールバック関数cb()を実行する。"); // メインのあいさつ
  cb(); // コールバック関数を実行
}
...
// 名前を表示する関数
function showName() {
  console.log(" ③追加処理 → Hi,Bob!。");
}

// ↓↓↓ 呼び出し元
// それぞれの関数をコールバックとして渡す
console.log("↓↓↓  ①関数greet(引数showName)。上記のメイン処理greetを実行する。");
greet(showName);
...
function greetWithCallback(callbackAction) {
  ...
  console.log(" Main-processingメイン処理 → Hell,hello,Tom!");
...
function sayGuestName() {
  ...
  console.log("addition追加関数① → Hell,hello,Ken!");
  // (6) ← 「Hi,Ken!」と表示して終了
}

// ▼ 追加関数②：ゲストの年齢を紹介
function sayGuestAge() {
  console.log("addition追加関数② → I'm,mmmmm 25 years old!");
}
...
console.log("↓↓↓ caller呼び出し元① → 名前を紹介する ===");
...
console.log("↓↓↓コールバック関数（例2-2）" ); 


console.log("↓↓↓ caller呼び出し元② → 年齢を紹介する ===");
