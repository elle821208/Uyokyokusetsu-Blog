...
// - idはページ内に一意（1つだけ）である必要があるため、基本的に「1対1の関係」で操作できる。
// - 同じく要素を取得できる querySelector("#foofoo") とほぼ同じ機能だが、
//   getElementById の方が「id取得に特化していて少しだけ高速」です。

// 🖥️ 補足説明：
// - この console.log(...) の結果として表示されるのは、
//   実際に取得された「HTML要素オブジェクト」です。
// - そのため、コンソール画面上には以下のように表示されます：
//
//     <div id="foofoo" style="background-color: gold"></div>
//
// - これは JS が取得した DOM 要素そのものをコンソールが「HTML形式」で表示しているためです。
//   つまり「文字列」ではなく「要素そのもの」が表示されているということ！
//
// ✅ このような表示は、開発時に実際のDOM構造を確認するのに便利です。
