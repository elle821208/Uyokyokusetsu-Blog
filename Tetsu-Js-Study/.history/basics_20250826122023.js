...
if (hasTicket && isAdult) {
  ...
  console.log(" ← もしも初期設定で hasTicket と isAdult が両方 true ならば、このメッセージが表示されます。");
} else {
  console.log("🚪 チケットがないか、子供なので入場できません… ❌");
  console.log(" ← 今回の初期設定では hasTicket と isAdult が両方 false またはどちらかが false なので、このメッセージが表示されます。");
}
