...
//
const pikachuget = () => {
    if(Math.random() <= 0.2){//20%以下かどうか
        return "「ピカチュウをゲットした！」";//当てはまる場合20%
    }else {//当てはまらない場合80%
        return "「ざんねん！もう少しでつかまえられたのに！」";
    }
};

console.log(pikachuget() + "");ボタンを押したら20%の確率で「ピカチュウをゲットした！」、
// 80%の確立で「ざんねん！もう少しでつかまえられたのに！」


//
document.querySelector("#button33").addEventListener("click", function () {
    //イベント 条件分岐
});
...
const hornDrill88 = (theirHp88) => {
    //関数hornDrill88 ()内に引数theirHp88 を設定する。{}内に処理。
    if (Math.random() <= 0.3) {
      //乱数を生成する。もしランダムな「0～0.999999…」の数字がが0.3以下の時は、(30%のときは)
      return theirHp88; //引数theirHpの値を返す
    } else {
      //上記と違う場合は
      return 0; //0を返す
    }
  ...
  console.log(hornDrill88(100) + "←関数hornDrill88の実行結果ですよ"); //呼び出し元。
  // ()は関数hornDrillの引数theirHpの値。仮で100とする。