...
if (hasTicket && isAdult) { // 両方 true なら実行
  ...
  console.log(`📝 条件判定: hasTicket = ${hasTicket}, isAdult = ${isAdult}  → 両方 true のじょうけんなので、このメッセージが表示される`);
} else {
  console.log("🚪 チケットがないか、子供なので入場できません… ❌");
  console.log(`📝 条件判定: hasTicket = ${hasTicket}, isAdult = ${isAdult}  → どちらかが false なので、このメッセージが表示される`);
}
