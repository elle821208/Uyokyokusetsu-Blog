...
function showNumber() {
  ...
  // ↓ただし、関数を「実行」しないとこの行は動かない。
}
// 🏠 (2-4) ここまでが関数 showNumber の中の世界（ローカルスコープ）。
// 🌟 この時点では「関数が作られただけ」でまだ動いていない！

//👇 =======================================
// 👇 (3) 関数を実行する（グローバルスコープから）
//👇 =======================================
// ↓ここは関数の外＝グローバルスコープ（全体の世界）です。
// グローバルスコープから関数を呼び出すことで、
// 関数の中の命令（ローカルスコープ）を動かすことができます。

showNumber(); 
// 👇 (3-1) 上で作った関数をここで「実行」する。
// すると、関数の中の命令（🏠 ローカル変数 number＝2）を動かすことができる。
// 🔁 つまり、関数の中のログがここで表示される。
...
// 🌏 (4-1) 結果：1 が表示される。
// なぜなら、ここは関数の外＝グローバルスコープ。
// 関数の中（🏠）で作った number（＝2）は、外では使えないため。
// → グローバル変数の number（＝1）が使われる。
