...
// 【例2】計算系コールバック関数 doubleFunction
function doubleFunction(n) { // コールバック関数 doubleFunction, コールバック引数 n
    // 第一引数 n = 数字（今回は呼び出し側より、ここでは skeletonFunction から渡される値、例: 5）

    // 【説明】
    // doubleFunction は skeletonFunction に渡される「コールバック関数」の一例
    // 骨組み関数 skeletonFunction は
    // 「どう処理するかは知らない、ただ渡された関数を呼び出すだけ」
    // なので、doubleFunction は「数字を2倍にする処理」を自由に行える

    // 例：
    // skeletonFunction(5, doubleFunction) と呼び出すと、
    // - num = 5 が doubleFunction の引数 n に渡される
    // - n * 2 = 10 の計算結果を出力する
