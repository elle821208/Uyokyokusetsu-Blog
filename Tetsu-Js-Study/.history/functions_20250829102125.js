...
function getDarkModeStatus(time) {
    // 作業の流れ:
    //  (1) 下記2.の const mode の右辺で new Date() により日時オブジェクトを作成
    //  (2) .getHours() で「時間（0～23）」だけを取り出す
    //  (3) その値（例: 21）が getDarkModeStatus(time) の time に代入される
...
//  (3) その値を引数 time として関数へ渡し、変数 mode に代入
...
// 3. 背景色を変更する処理
// 作業の流れ:
//  (1) const mode に格納された値（今回は "dark"）を if 文で判定
//  (2) mode が "dark" なら黒、"light" なら白に背景色を切り替える
...
function getSeason(month) {
    ...
    //  (3) +1 して「1～12」の形式に直す
    //  (4) その値（例: 8）が getSeason(month) の month に代入される
...
// 2. メソッドで値を厳選（.getMonth()）
// 3. 値を整形（+1）して変数に代入
// 4. const season の右辺で関数呼び出しを行い、引数に値を渡す
