...
const getInitialname = (name) => {//引数nameに姓名（半角区切り）を渡すとイニシャルが返ってくる関数
    const nameArray = name.split("");//splitで値を分割する。
    const initialLast = nameArray[0].slice(0);//引数を姓
    const initialFirst = nameArray[1].slice(0);//引数（名）
    return initialLast + "." + initialFirst;
}

const result55 = getInitialname("Yamada Hiroki");//今回引数の値は"Yamada Hiroki"。返ってくる期待の値はイニシャルの"Y.H"
