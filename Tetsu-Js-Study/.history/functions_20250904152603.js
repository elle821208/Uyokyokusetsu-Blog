...
// 1️⃣ 呼び出し側で関数を実行
// skeletonFunction(10, judgeFunction)

// 2️⃣ 引数の流れ（呼び出し側引数 と 骨組み関数引数）
// ┌───────────────┐
// │ 第一引数 num    │ ← 10
...
// 3️⃣ 骨組み関数内で処理
// actionFunction(num) を実行
// → これは実際には、呼び出し側の引数により judgeFunction(10) に置き換えられる

// 4️⃣ 結果
// judgeFunction(10) が実行される
// =======================================
// 🚩STEP2: 骨組み関数（メインの流れだけ決める関数 = 高階関数）
// =======================================

// 骨組み関数 skeletonFunction
function skeletonFunction( // 骨組み関数
    num,               // 第一引数 num = 数字
    actionFunction     // 第二引数 actionFunction = コールバック引数（外から渡す処理関数）
) { 
    // 【説明】
    // 骨組み関数 skeletonFunction は
    // 「数字を受け取って誰かに渡す」役だけをする。
    // 具体的に「どう判定するか」「どう計算するか」は知らない。
    // その部分は外から渡されたコールバック引数 actionFunction に任せる。

    // 例：
    // 呼び出し側 skeletonFunction(10, judgeFunction) の場合
    // 第一引数 num = 10、第二引数 actionFunction = judgeFunction となる。
    // そのため骨組み関数内の コールバック引数 actionFunction(num) は
    // → コールバック関数 judgeFunction(10) に置き換わる
    // そして judgeFunction(10) が実行される

    actionFunction(num);  
    // ✅ 骨組み関数 skeletonFunction の中でコールバック引数 actionFunction を呼び出す
    // （actionFunction = judgeFunction のため、実際には コールバック関数 judgeFunction(num) が呼ばれる）
}
...
// =======================================
console.log("↓↓↓ 「コールバック関数を差し替えると、同じ骨組み関数から判定処理にも計算処理にもできる」 ↓↓↓");
// 💡 コールバック関数を差し替えると：  
//    - 判定処理（judgeFunction）にも  
//    - 計算処理（doubleFunction）にもできる  
// → 同じ骨組み関数でも結果が変わる



// 【例1】判定系コールバック関数 judgeFunction
function judgeFunction(n) { // コールバック関数 judgeFunction, コールバック引数 n
    // n は数字（今回は呼び出し側より、ここでは 10 や 3 が渡される）

    // 例：呼び出し側で skeletonFunction(10, judgeFunction) とすると
    // コールバック引数 n = 10 が judgeFunction に渡される
    // 「10 > 5」を判定することになる

    if (n > 5) {
        console.log("判定関数 → 大きい！（今回は n が 5 より大きいから）" + " ← 判定系コールバック関数より");
    } else {
        console.log("判定関数 → 小さい！（今回は n が 5 より大きいから）" + " ← 判定系コールバック関数より");
    }
}



// 【例2】計算系コールバック関数 doubleFunction ※ 下記の呼び出し入の三つ目 入力値 5 の場合 で 計算系コールバック関数 doubleFunction を呼び出している。
function doubleFunction(n) { // コールバック関数 doubleFunction, コールバック引数 n
    // n は呼び出し側から渡された数字（例: 5）

    // 【説明】
    // doubleFunction は skeletonFunction に渡される「コールバック関数」の一例
    // 骨組み関数 skeletonFunction は
    // 「どう処理するかは知らない。渡された関数を呼び出すだけ」
    // なので、doubleFunction では「数字を2倍にする処理」を自由に行える

    // 例：
    // skeletonFunction(5, doubleFunction) と呼び出すと
    // - 引数 n = 5 が doubleFunction に渡される
    // - n * 2 = 10 を計算する
    // → コンソールには "計算関数 → 2倍 = 10" と表示される

    console.log("計算関数 → 2倍 =", n * 2 + " ← 計算系コールバック関数より");
}



// =======================================
// 📝 補足：judgeFunction と doubleFunction の違い
// =======================================
// - judgeFunction（コールバック関数, 判定系処理）
//    入力された数字を「5より大きいかどうか判定」して結果を出力する処理
//    （条件分岐を使った判定型の処理）
//
// - doubleFunction（コールバック関数, 計算系処理）
//    入力された数字を「2倍にして出力」する処理
//    （計算に特化した処理）
//
// 👉 どちらも skeletonFunction（骨組み関数）に渡せるが、
//    役割が「判定系」と「計算系」で異なる点が重要。
...
// =======================================
// 💡 ポイント
// - if文版（fixedJudgeFunction）は「5より大きいかどうか判定」しかできない（固定処理関数）
// - コールバック版（skeletonFunction + judgeFunction/doubleFunction）は
//   「判定」や「2倍」など外から処理を自由に差し替え可能（骨組み関数 + コールバック関数）
// =======================================

console.log("=== 入力値 10 の場合 if文と判定系コールバック関数を使う ===");
fixedJudgeFunction(10);                     // if文版（固定処理関数 fixedJudgeFunction）
skeletonFunction(10, judgeFunction);        // コールバック版（骨組み関数 skeletonFunction + 判定系コールバック関数 judgeFunction）

console.log("=== 入力値 3 の場合 if文と判定系コールバック関数を使う ===");
fixedJudgeFunction(3);                      // if文版（固定処理関数 fixedJudgeFunction）
skeletonFunction(3, judgeFunction);         // コールバック版（骨組み関数 skeletonFunction + 判定系コールバック関数 judgeFunction）

console.log("=== 入力値 5 の場合 上記二つと異なり、if文と計算系コールバック関数を使う===");
fixedJudgeFunction(5);                      // if文版（固定処理関数 fixedJudgeFunction）
skeletonFunction(5, doubleFunction);        // コールバック版（骨組み関数 skeletonFunction + ※※※上記二つと異なり、計算系コールバック関数 doubleFunction）
...
//
// === 入力値 10 の場合 ===
// if文 → 大きい（固定処理関数 fixedJudgeFunction による判定）
// コールバック → 大きい（骨組み関数 skeletonFunction + 判定系コールバック関数 judgeFunction）
//
// === 入力値 3 の場合 ===
// if文 → 小さい（固定処理関数 fixedJudgeFunction による判定）
// コールバック → 小さい（骨組み関数 skeletonFunction + 判定系コールバック関数 judgeFunction）
//
// === 入力値 5 の場合 ===
// if文 → 小さい（固定処理関数 fixedJudgeFunction による判定）
// コールバック → 2倍 = 10（骨組み関数 skeletonFunction + 計算系コールバック関数 doubleFunction）
//
// =======================================
// 📝 違いまとめ
// - if文版（固定処理関数 fixedJudgeFunction）：ルールが固定で差し替え不可
// - コールバック版（骨組み関数 skeletonFunction + コールバック関数 judgeFunction/doubleFunction）
//   ：同じ骨組みでも外から処理を差し替え可能
// =======================================
