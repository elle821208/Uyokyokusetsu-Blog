...
console.log(document.getElementsByClassName("barbar")[2].innerHTML = "<div>同様に配列を編集した。これは[2]番目。※※こｒgetElementsByClassName（クラスのみ有効）で取得した。</div>");//classを全て取得する。配列になる。

///<!--2:04:30～-->
const $zoozoo = document.querySelectorAll(".barbar");//変数に4つのクラス（querySelectorAll（.barbarの4つのクラス））を格納しておく。
$zoozoo[3].innerHTML = "<div>変数$zoozooをinnerHTMLで編集する。あらかじめ変数$zoozooにquerySelectorAll（.barbarの4つのクラス）を格納している。これは[4]番目。</div > ";