...
function checkNumber_if(num) {
    // この関数は「5より大きいかどうか」しか判定できない
    if (num > 5) {
        console.log("if文 → 大きい");
    } else {
        console.log("if文 → 小さい");
...
// 呼び出し例
checkNumber_if(10); // → if文 → 大きい
checkNumber_if(3);  // → if文 → 小さい
...
function checkNumber_cb(num, callback) {
    // callback に外から渡された処理を実行する
    callback(num);
}
...
// 呼び出し例
checkNumber_cb(10, biggerOrSmaller); // → コールバック → 大きい
checkNumber_cb(3, biggerOrSmaller);  // → コールバック → 小さい
checkNumber_cb(5, doubleNumber);     // → コールバック → 2倍 = 10
