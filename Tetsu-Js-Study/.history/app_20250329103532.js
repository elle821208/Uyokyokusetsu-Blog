...
//
//関数(アロー関数と関数式)
const area = (height, width) => {//アロー関数式area(引数を設定)
    return height * width;//アロー関数areaの戻り値(関数areaの実行内容)。下記呼び出し元に値が返る。
};


//呼び出し元。コンソールで関数areaを呼び出す。引数の値は12、5を設定しておく。
//出力する際にアロー関数areaの戻り値(関数areaの実行内容)が代入される。
console.log(area(12, 5));//①呼び出し元の引数
//60
