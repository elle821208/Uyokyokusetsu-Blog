...
for (var i = 0; i < lists.length; i++) {//初期値:i変数の最初の配列の数は0、条件式iが配列listsの配列の数より小さい時、増減式i++で配列の数iが1ずつ増ええていく
...
for (let index = 0; index < questions.length; index++) {//初期値:変数indexの最初の配列の数は0、条件式iが配列indexの配列の数より小さい時、増減式i++で配列の数iが1ずつ増ええていく
...
const pikachu2 = {
  ...
  levelUp: function () {
    //メソッドlevelUp。{}の中の処理にif文を置き繰り返し処理の値を作る。
      let ar = [10, 20, 30];//配列ar
      for (let i = 0; i < ar.length; i++) {//初期値：変数iで最初の配列の数は0、条件式iが配列arの配列の数より小さい時、増減式i++で配列の数iが1ずつ増ええていく
      console.log(ar[i]);//配列arの変数iの数(繰り返した数)だけ呼び出す。
    }
  },
};

//呼び出し元。オブジェクトpikachu2のlevelUpメソッドを実行する。
console.log(pikachu2.levelUp + "ピカチューのレベルが20まで上がると…");



//chatgtpより
//メソッドを実行すると値が1ずつ増える
const counter = {//オブジェクトcounter。{}内はプロパティ情報。
    value: 0, // プロパティ名value。初期値
    increment: function() {//メソッド名increment。{}内は処理。for文で繰り返す。
      for (let i = 0; i < 1; i++) { // 変数i。初期値:変数iはさいしょ、条件式、増減式:変数iを1ずつ増やす。1回ループ
        this.value++; // 値を1増やす
      }
...
//上記の配列をfor文で繰り返す。
for (let index = 0; index < questions2.length; index++) {//初期値:変数indexの最初の配列の数は0、条件式:question2の配列の数より小さい時、増減式:index配列の数を1ずつ増やしていく
