...
// =======================================
// 👤 オブジェクト snsUser2 を定義する
// =======================================
// 「オブジェクト」とは、複数のデータ（プロパティや関数）をひとまとめにできる入れ物。
// 人やモノの「属性（id や userName）」と「動き（メソッド）」をセットで表現できます。
...
const snsUser2 = {
  ...
  post: function (contents) {
    // (1-4) メソッド post を定義。これは「投稿する動作」を表す関数。
    ...
    // ✅ 【this を使うメリット】
    // - 他のユーザーオブジェクトにも同じメソッドをコピーできる（再利用性◎）。
    // - userName の値が変わっても、this.userName によって常に最新の値を使える。
    // - 複数のオブジェクトが存在しても、それぞれ自分の userName を正しく参照できる。

    // ❌ 【this を使わなかった場合】
    // たとえば次のように書いた場合：
    //   return contents + "を投稿しました by…" + "Tomoko";
    // - ユーザー名が固定されてしまい、他のユーザーには使い回せない（再利用性 ×）。
    // - userName を後から変更しても、反映されない（柔軟性 ×）。
  },
};
...
console.log("呼び出し元。snsUser2.post(引数：プログラミングなう。) で 上記のsnsUserオブジェクトのpost関数をreturnで呼び出す。returnのところで 引数contents + を投稿しました by… + this.userNameでこのオブジェクトsnsUser2のuserNameプロパティTomoko →" + snsUser2.post("プログラミングなう。"));
// (2-1) snsUser2 オブジェクトの post メソッドを呼び出す。
// (2-2) 引数として "プログラミングなう。" を渡して実行。
//       この値は、メソッド内の contents に渡される。
// (2-3) 実行すると、return により投稿メッセージが生成されて返ってくる。
// (2-4) 結果：コンソールに「プログラミングなう。を投稿しました by…Tomoko」と表示される。
