...
const pikachu2 = {
  ...
  //       これはポケモン「ピカチュウ」の情報をまとめたもの。

  name: "ピカチュウ",
  // (1-2) プロパティ name。文字列 "ピカチュウ" を格納。
  //       ポケモンの名前を示す。
  ...
  levelUp: function () {
    ...
    // (1-7) インクリメント演算子（++）を使って、
    //       現在のレベルを 1 増加させる。
    //       this.level は、このオブジェクト（pikachu2）の level を指す。
    ...
    // - レベルアップするたびに、元の状態の値を直接上書きして記録するので、
    ...
    if (this.level >= 20) {
      ...
      this.skills.push("スパーク");
      // (1-9) 新しい技「スパーク」を skills 配列の末尾に追加する。
      //       push() は、配列の最後に要素を追加するためのメソッド。
    }

    // ✅【this の役割】
    // - this.level → このオブジェクト（pikachu2）の level を参照。
    // - this.skills → このオブジェクトの skills 配列を参照。
    // - つまり「どのインスタンス（個体）の情報を操作しているか」を示す仕組み。
    // - 他のポケモンオブジェクトにこのメソッドをコピーしても機能する（再利用性◎）。
  },
};
...
console.log(
  "📢 呼び出し元 console。" +
  "↑ 上の行でオブジェクト pikachu2 のメソッド levelUp() をもう一度実行しました。" +
  "this.level++ によってレベルが 20 に上がり、" +
  "if 文の条件（level >= 20）を満たしたため、" +
  "新しいスキル『スパーク』が skills に追加されました。→",
  pikachu2.level,
  pikachu2.skills
);
