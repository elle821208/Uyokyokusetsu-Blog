...
console.log("↓↓↓ === アロー関数 hyperDrive を定義する。処理に戻り値7777を入れる。 ===");
const hyperDrive = () => {
  return 7777; 
...
// --------------------------------------------------
console.log("↓↓↓ const resultValue = hyperDrive(); で 定数 resultValueを定義して 関数hyperDrive を実行する。"
    + "下記のconst resultValueでは 関数名 hyperDrive の後ろに () をつけると"
    + "関数hyperDrive実行され、戻り値7777が resultValue に入る。");
const resultValue = hyperDrive(); 
// 上記のconst resultValueでは 関数名 hyperDrive の後ろに () をつけると関数hyperDrive実行され、戻り値7777が resultValue に入る
...
console.log(`${resultValue} ← 左記の波括弧resultValue で 定数 resultValue を
    参照(変数に入っている値をそのまま使う)する。
...
// 関数名そのものを直接呼び出す例
// --------------------------------------------------
console.log(`${hyperDrive()} ← $波括弧hyperDriveカッコ で アロー関数 hyperDrive() を直接呼び出した。`);
...
console.log(`${resultValue2} ← 左記の波括弧resultValue2 で 定数 resultValue2 を直接呼び出した。`);
