...
//あるif条件式のコードを…<!--1:21:40～-->
if (new Date(), GetHours() > 20) {
    document.body.style.backgroundColor = "#000";
}

//上記のif条件式をfunctonで関数化
//関数名はchageDarkMode
//20時にダークモードになる処理
...
//関数に引数を渡す。
//上記の関数chageDarkModeは20時固定だが、
//冬は18時間でそれ以外は20時にしたい場合は
//下記の引数を設定する。
//<!--1:23:15～-->
function changeDarkMode(time = 20) {//引数time、デフォルト値20を設定する。
    if (new Date(), GetHours() > time) {//引数timeを設定する。
        document.body.style.backgroundColor = "#000";
    }    
}
...
//コールバック関数<!--1:26:05～-->
function hoo(callback) {//callback関数をつける
    console.log("Hi,Tom!");
    callback();//callback関数をつける
}

function ioo() {
    console.log("Hi,Ken!");//callback関数をつける
}
