...
function getDarkModeStatus(time) {
    // 💡 getDarkModeStatus(new Date().getHours()) の流れ
    ...
    console.log(`👉 引数 time に渡された値: ${time} （今回の値: 21）`);
...
// 2. 関数を呼び出して結果を変数に格納
// 💡 new Date() → 現在の日時を生成するオブジェクト
// 💡 getHours() → 現在の時間（0～23）を返す
//    例: 2025年8月28日 21時の場合 → 21 が返る
const mode = getDarkModeStatus(new Date().getHours()); 
console.log(`👉 new Date().getHours() の値: ${new Date().getHours()} （今回の値: 21）`);
console.log(`👉 関数 getDarkModeStatus に渡された time の値: ${mode === "dark" ? 21 : 0} （今回の値: 21）`);
...
// 4. 結果をコンソールに表示
console.log(`現在のモード: ${mode} （今回の値: dark）`);

// =======================================
// まとめ
// =======================================
// 1. new Date().getHours() で現在の時間（0～23）を取得 → 今回は 21
// 2. getDarkModeStatus(time) を呼び出す → getDarkModeStatus(21)
// 3. 引数 time に 21 が渡され、if 文で判定
// 4. time > 20 なので "dark" が返される
// 5. 戻り値 "dark" が mode に代入され、背景色変更や出力に利用される
// =======================================
// JavaScript応用例：背景色変更（引数・戻り値付き）
// =======================================
...
// 5️⃣ 戻り値 "dark" が mode（const 定数）に代入され、背景色変更や出力に利用される
...
// 💡 new Date() → 現在の日時を取得
// 💡 getMonth() → 月を0始まり（1月=0, 2月=1,...,12月=11）で取得
// 💡 +1 することで人間が普段使う「1～12月」の形式に直す
//    2025年8月28日 現在: getMonth() = 7 → +1 → 8
console.log(`👉 現在の月 (currentMonth): ${currentMonth} （今回の値: 8）`);

// 3. 関数を呼び出して季節を取得
const season = getSeason(currentMonth);
console.log(`👉 関数 getSeason に currentMonth (${currentMonth}）を渡した結果: ${season} （今回の値: 夏）`);

// 4. 結果をコンソールに表示
console.log(`現在の月は ${currentMonth} 月です （今回の値: 8）`);
console.log(`🌟 なので、季節は ${season} です！ （今回の値: 夏）`);

// =======================================
// まとめ
// =======================================
// 1. currentMonth に「今の月（1～12）」が入る → 今回は 8
// 2. getSeason(currentMonth) を呼び出す → getSeason(8)
// 3. currentMonth の値（8）が引数 month に渡される
// 4. 関数内で month=8 をもとに季節を判定 → 夏
// 5. 戻り値「夏」が season に代入され、表示される
