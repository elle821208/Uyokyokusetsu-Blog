...
// ---------- &&（かつ）の例 ----------
console.log(1↓↓↓ 初期設定。今回は ${hasTicket} = true と isAdult = ${isAdult} が両方 true');
...
if (hasTicket && isAdult) { // 両方 true なら実行
  ...
  console.log(` ↑↑↑ 今回の初期設定にあてはまらない。hasTicket = ${hasTicket} かつ isAdult = ${isAdult} が両方 true なので表示されない。`);
} else {
  console.log("🚪 チケットがないか、子供なので入場できません… ❌");
  console.log(` ↑↑↑ 今回の初期設定にあてはまる。hasTicket = ${hasTicket} または isAdult = ${isAdult} が false なので表示される。`);
}
