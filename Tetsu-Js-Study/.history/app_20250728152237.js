...
let globalNumber = 1; // (1-1) グローバル変数globalNumberを作り、1を入れる。

// =======================================
// (2) 関数 showLocalNumber を作る
// =======================================
// 関数は「手順書」や「レシピ」のようなもの。
// この関数を使うと、特定の処理をまとめて実行できる。

function showLocalNumber() {
  // (2-1) ここから関数showLocalNumberの中の世界。
  // 関数の中で作る変数は「ローカル変数」。
  // ローカル変数はこの関数の中でしか使えない。

  const localNumber = 2; // (2-2) 関数の中だけで使えるlocalNumberを作り、2を入れる。

  console.log("関数showLocalNumberの中。で使える変数LocalNumberの値→" +localNumber); // (2-3) localNumberの値「2」を表示。
}
// (2-4) ここまでが関数showLocalNumberの中の世界。

// =======================================
// (3) グローバル変数の値を表示
// =======================================
console.log("関数showLocalNumberの外。で使える変数LocalNumberの値→" +globalNumber); 
// (3-1) 関数を実行していないので、ここではglobalNumber「1」が表示される。
