...
console.log(`🎯 ↑↑↑ 初期設定 → hasTicket = ${hasTicket}, isAdult = ${isAdult}`);

// 「チケットを持っていて、なおかつ大人なら入場できる」
if (hasTicket && isAdult) { // 両方 true なら実行
  console.log("🎫 チケットもあって大人なので入場できます！ ✅");
  console.log(`📝 条件判定:今回の初期設定は hasTicket = ${hasTicket}, isAdult = ${isAdult}  なので→ 初期設定で両方 true の場合は、このメッセージが表示される`);
} else {
  console.log("🚪 チケットがないか、子供なので入場できません… ❌");
  console.log(`📝 条件判定: 今回の初期設定は hasTicket = ${hasTicket}, isAdult = ${isAdult}  なので→ 初期設定でどちらかが false 場合は、このメッセージが表示される`);
}
