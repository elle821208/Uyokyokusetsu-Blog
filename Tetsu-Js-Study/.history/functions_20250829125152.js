...
//
// 💡 引数の考え方（＝「値を受け渡す仕組み」）
// - 実引数: 関数を呼び出すときに渡す「具体的な値」
//   例: getSeason(8) の「8」や、currentMonth が 8 の場合 → 8 が渡される
//
// - 仮引数: 関数が受け取る側で使う「受け皿の変数名」
//   例: function getSeason(month) { ... } の month が仮引数
//
// 👉 イメージ: 実引数 = プレゼントの「中身」🎁、仮引数 = それを受け取る「箱」📦
//    getSeason(currentMonth) と書いたとき、currentMonth の値 (8) が「箱」month に入る
...
function getDarkModeStatus(time) {
    ...
    //  (3) その値（今回の設定の2025年8月28日より 21）が getDarkModeStatus(time) の time に代入される
...
//  (3) その値（今回の設定の2025年8月28日より 21）を引数 time として関数へ渡し、変数 mode に代入

// 3. 背景色を変更する処理
// 作業の流れ:
//  (1) const mode に格納された値（今回の設定の2025年8月28日より "dark"）を if 文で判定
...
function getSeason(month) {
    ...
    //  (3) +1 して「1～12」の形式に直す
    //  (4) その値（今回の設定の2025年8月28日より 8）が getSeason(month) の month に代入される
    //     → このとき、currentMonth (実引数 🎁) が month (仮引数 📦) に渡される
...
//  (3) +1 して「1～12」の形式に変換
//  (4) その値（今回の設定の2025年8月28日より 8）を変数 currentMonth に格納

// 3. 関数を呼び出して季節を取得（season は const 定数）
const season = getSeason(currentMonth);
// 作業の流れ:
//  (1) const season の右辺で currentMonth の値（今回の設定の2025年8月28日より 8）を引数として getSeason(month) に渡す
//      → 実引数 currentMonth (🎁 8) が仮引数 month (📦) に代入される
...
// 3. 値を整形（+1）して変数に代入
// 4. const ～ の右辺で関数呼び出しを行い、実引数 🎁 を仮引数 📦 に渡す
