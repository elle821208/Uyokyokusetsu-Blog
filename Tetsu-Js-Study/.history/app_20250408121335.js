...
console.log(document.querySelectorAll(".barbar")[1].innerHTML = "<div>.barbarクラスの配列の中身を[番号]で指定し、innerHTMLで編集する。これは[1]番目。※※querySelectorAll（idもクラスも取得可）で取得した。</div>");
...
///<!--2:08:03～-->
////空っぽのHTMLクラスにappendChildで情報を持った変数を入れ込む。
//↓↓↓ DOM用（HTMLのクラス用）の情報（タグの種類、クラス、クラス名、文章"おなか減ったなう"）を入れる
    const $post = document.createElement("article");//変数$postを宣言する。createElementでDOM用のHTMLタグを生成する。（今回は引数どおりのarticleタグを生成する。）
...
//↓↓↓ HTMLからtimelineクラス（この段階ではまだ情報は空っぽ）を取得し、上記の$postの情報を入れる。
    const $timeline = document.querySelectorAll(".timeline")[0];//変数timelineを宣言する。querySelectorAllでHTMLに記述した.timeline(引数)クラスの[]番目を取得する。
...
window.addEventListener("load", function () {///addEventListener第一引数（イベントのタイプ）、第二引数（イベント時に実行する関数）
    alert("読み込み完了");
});


/// ボタンを押すと"クリックされた！"がコンソール出力されるイベントを設定する。
document.querySelector("#button").addEventListener("click", function () {///querySelectorでHTMLのid="button"を取得する。
...
//まず$pikachu22(id=output用の情報、タグ、クラス名など)を作る
const $pikachu22 = document.createElement("article");///DOM操作。outputクラス用にarticleタグを作る。
$pikachu22.setAttribute("class", "pikachu22");  ///DOM操作。outputクラス用の情報（$pikachu22）を作る。
//空のid=outputに$pikachu22を入れて、タグ、クラス、クラス名などを追加する。
const $output = document.querySelectorAll("output");//HTMLのid="output"を取得
$output.appendChild("$pikachu22")//$outputに$pikachu22（クラス用の情報）を入れる


//クリックしたら$outputが表示される
