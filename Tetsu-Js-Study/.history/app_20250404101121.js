...
for (var i = 0; i < lists.length; i++) {//初期値:i変数の最初の配列の数は0、条件式iが配列listsの配列の数より小さい時、増減式i++で配列の数iが1ずつ増ええていく
  console.log(lists[i]); //配列listsのiの数(繰り返した数だけ)を呼び出す。
}
...
for (let index = 0; index < questions.length; index++) {//初期値:iの最初の配列の数は0、条件式iが配列indexの配列の数より小さい時、増減式i++で配列の数iが1ずつ増ええていく
  console.log(questions[index]);//配列questionsのindexの数(繰り返した数)だけ呼び出す。
}
//let index = 0;で↓
...
const pikachu2 = {
  ...
  levelUp: function () {
    //メソッドlevelUp。{}の中の処理にif文を置き繰り返し処理の値を作る。
      ...
      console.log(ar[i]);//配列arのiの数(繰り返した数)だけ呼び出す。
    }
  },
};

//呼び出し元。オブジェクトpikachu2のlevelUpメソッドを実行する。
console.log(pikachu2.levelUp);
