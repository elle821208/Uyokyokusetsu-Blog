...
//コールバック関数<!--1:26:20～-->
console.log("↓↓↓コールバック関数（例１）" ); 

// メイン関数：あいさつのあとに追加処理（コールバック関数）を実行する
function greet(cb) {
    console.log(" ②メイン関数greet(cb)。メイン処理 → Hi,Amy!。"
        + "この時点で、呼び出し元とメイン関数により "
        + "greet(showName) = greet(cb)"
        + "よってshowName = cb となる。"
        + "上記のメイン関数の引数(cb)を "
        + "③下記でコールバック関数cb() として実行する。"
        + "つまり下記のcb()で関数showNameを実行している"); // メインのあいさつ
  cb(); // コールバック関数を実行
}
...
// 名前を表示する関数
function showName() {
    console.log(" ③追加関数showName。※ 追加関数を複数設定して追加処理を増やすことができる。"
        + "追加処理 → Hi,Bob!。");
}

// ↓↓↓ 呼び出し元
// それぞれの関数をコールバックとして渡す
console.log("↓↓↓  ①下記でメイン関数を実行する。""メイン関数greet(追加関数showName)。※ ()内の追加関数を複数設定して追加処理を増やすことができる。"
    + "下記でメイン関数greet(追加関数showName)を実行する。上記のメイン処理greet(cb)を実行する。");
greet(showName);
...
function greetWithCallback(callbackAction) {
  ...
  console.log(" メイン処理 → Hell,hello,Tom!");
  // (3) ← メイン処理。まず自分のあいさつ「Hi,Tom!」を表示する。

  callbackAction();
  // (4) ←  メイン関数の引数 (callbackAction) を 関数callbackAction()として実行。
...
function sayGuestName() {
  ...
  console.log("追加関数① → Hell,hello,Ken!");
  // (6) ← 「Hi,Ken!」と表示して終了
}

// ▼ 呼び出し元（ユーザーが自由に関数を指定できる！）
console.log("↓↓↓ 呼び出し元① → 名前を紹介する ===");
