...
// =======================================
// JavaScriptの基本：関数（function）の使い方（背景色変更例）
// =======================================

// 1. 通常の関数の定義
function changeDarkMode() {
    // 現在の時刻を取得して20時以降なら背景を黒にする
...
// 👉 補足説明
console.log("👉 上記で関数名 changeDarkMode() を実行");
console.log("   - 関数内の if 文で現在の時刻が 20 時より大きい場合に背景が黒くなる");
console.log("👉 関数は呼び出さないと処理は実行されない。上記で changeDarkMode() を呼び出したので処理が実行された");

// 2. 関数式（無名関数）として書く例
const changeDarkMode2 = function() {
    if (new Date().getHours() > 20) {
        document.body.style.backgroundColor = "#000";
    }
};
...
// 👉 補足説明
console.log("👉 上記で関数名 changeDarkMode2() を実行");
console.log("   - 関数式は変数に代入して使う形で、必要なときに呼び出す");
