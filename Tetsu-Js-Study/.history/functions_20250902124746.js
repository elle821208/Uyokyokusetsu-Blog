...
// =======================================
// 🟢 このコードの目的
// 「メイン関数の中での追加処理」と「コールバック関数の中での追加処理」の違いを理解する
// =======================================

console.log("↓↓↓ a = メイン関数（固定の骨組み）、b = コールバック引数（a に渡される）、c = コールバック関数（差し込める処理、b が指すもの） ↓↓↓");

// =======================================
// 🚩STEP2: メイン関数定義 a（呼び出される側、骨組み部分）
// =======================================
function a(b) { // a = メイン関数, b = コールバック引数（ここに c が渡される）
    // 【説明1】
    // 呼び出し側に a(c); と呼び出されたとき、
    // メイン関数 a(b) の コールバック引数 b に「コールバック関数 c」が渡される
    ...
    console.log("【2】メイン関数の追加処理（a 内）：毎回必ず実行される");
    ...
    console.log("👉 最後にコールバック関数 c() を呼び出す（b = c のため b() が c() を実行）");
    b();  // b() は c() を実行する
}
...
// 🚩STEP1: 呼び出し（実行開始）
// =======================================
a(c); // a = メイン関数 に c = コールバック関数 を渡す（b = c） 
...
console.log("↓↓↓ f = メイン関数（固定の骨組み）、g = 引数、i = 数字（固定の値） ↓↓↓");

// =======================================
// 🚩STEP2: メイン関数定義 f（呼び出される側）
// =======================================
function f(g) {
    console.log("【1】メイン処理：必ず実行される基本の動作（骨組み部分）");

    console.log("【2】メイン関数の追加処理：毎回必ず実行される（まとめや後片付けのイメージ）");
...
// =======================================
// 🟢 if文 vs コールバック関数（シンプル版）
// =======================================

console.log("↓↓↓ j = メイン関数、k = 引数、l = コールバック関数 ↓↓↓");

// =======================================
// 🚩 if文入り関数（固定ルール、差し替え不可）
// =======================================
// if文版
function j_if(k) {
    if (k > 5) {
...
// コールバック版
function j_cb(num, l) {
    console.log("【メイン j】入力値 =", num);
    l(num); // コールバック関数呼び出し
    console.log("【メイン j】骨組み処理完了");
}

// コールバック関数例
function l1(n) {
    if (n > 5) {
        console.log("【コールバック】大きい");
    } else {
        console.log("【コールバック】小さい");
    }
}

function l2(n) {
    console.log("【別コールバック】2倍 =", n * 2);
}
...
// メリットまとめ:
// - if文 = 動き固定、書き換え必要
// - コールバック = 外から自由に差し替え可能、柔軟性抜群
// =======================================
// 🟢 if文 vs コールバック関数（超ていねい解説付き）
// =======================================

console.log("↓↓↓ j_if = if文版関数、j_cb = コールバック版関数 ↓↓↓");

// =======================================
// 🚩 if文入り関数（処理が固定されているパターン）
// =======================================
//
// ・if文を使うと「関数の中のルール」が最初から決まっていて
//   呼び出すときに変えることができない。
// ・つまり「毎回同じ基準で動く」しかできない。
//
function j_if(k) { // j_if = if文入り関数
    // 入力された数字が 5 より大きいかどうかを判定するルール
    if (k > 5) {
        console.log("【if文】大きい"); // 条件に合えばこっち
    } else {
        console.log("【if文】小さい"); // 合わなければこっち
    }
}

// 呼び出し例（この関数は「5より大きいかどうか」しか判定できない）
j_if(10); // → 【if文】大きい
j_if(3);  // → 【if文】小さい
...
// =======================================
// 🚩 コールバック関数版（柔軟に差し替え可能なパターン）
// =======================================
//
// ・コールバック関数を使うと「判定のルールや処理の内容」を
//   呼び出すときに外から渡せる。
// ・つまり「関数の骨組みは固定」だけど「中の動作」は差し替え可能。
// ・そのため、同じ j_cb という関数を使っても、渡すコールバック次第で
//   全然違う動きをさせられる。
//
function j_cb(num, callback) { // j_cb = メイン関数, callback = コールバック関数
    console.log("【メイン j_cb】入力値 =", num);

    // 💡 ここがポイント！
    // callback(num) の部分で「外から渡された関数」を実行する。
    // → 呼び出すたびに違う動きをさせられる。
    callback(num);

    console.log("【メイン j_cb】骨組み処理完了");
}



// =======================================
// 🚩 コールバック関数の例（差し替えられる動作）
// =======================================

// コールバック関数例1（if文と同じように「大きい/小さい」を判定）
function l1(n) {
    if (n > 5) {
        console.log("【コールバック】大きい");
    } else {
...
// j_cb に「大きい/小さい判定（l1）」を渡す
j_cb(10, l1); // → 【コールバック】大きい
j_cb(3, l1);  // → 【コールバック】小さい

// j_cb に「2倍する処理（l2）」を渡す
j_cb(5, l2);  // → 【別コールバック】2倍 = 10
...
//   - ルールが関数の中に書き込まれている
//   - 呼び出すときに動きを変えられない（固定）
//   - 「5より大きいか小さいか判定する」だけの関数
//
// ● コールバック版（j_cb）
//   - 骨組み（流れ）は決まっているが、具体的な動作は外から渡せる
//   - 呼び出すたびに「どんな処理をするか」を差し替え可能（柔軟）
//   - 今回は「大きい/小さい判定」や「2倍計算」などに変えられた
//
// =======================================
