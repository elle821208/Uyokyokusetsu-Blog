...
const getInitialname = (name) => {//引数nameに姓名（半角区切り）を渡すとイニシャルが返ってくる関数
    const nameArray = name.split("");//変数nameArray。splitメソッドで引数の値"Yamada Hiroki"を分割する。
    console.log("nameArray:" , nameArray);
    const initialLast = nameArray[0].slice(0);//変数initialLast。上記の値を配列の[0]番目としてsliceメソッドで抽出する。引数（姓）
    console.log("initialLast:" , initialLast)
    const initialFirst = nameArray[1].slice(0);//引数（名）
    console.log("initialFirst:" ,)
