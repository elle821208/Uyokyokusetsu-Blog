...
//
//関数(アロー関数と関数式)
const area = (height, width) => {//②アロー関数式area(引数を設定)。呼び出し元の引数の値が代入される。
    return height * width;//③戻り値。関数areaの実行内容=戻り値。が実行され戻り値になる。
};


//呼び出し元。関数areaを呼び出す。引数の値は12、5を設定しておく。
//出力する際に戻り値(関数areaの実行内容)が代入される。
