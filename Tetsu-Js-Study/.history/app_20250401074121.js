...
console.log(moo);
//いくつかのカッコが組み合わさっていますが、その一番内側から見ていきます。
// Math.random()*6/６種類の結果が欲しいので、
// Math.random()に6を掛けています。
//すると、この部分の数値は「0～5.999999…」の範囲内の、
// いずれかの数値、という事になります。（この時点では小数のある数値なので膨大なパターンの結果になりえます）
...
// Math.floorの引数にする事で、「1以上=30%」は1(1は当たり）、「1未満=70%」は0（0はハズレ）と分かれる事になりますが、その場合の「1以上」になる確率は前述の通り「足した小数に準ずる」事になります。
