...
// コールバック関数<!--1:26:20～-->
// =======================================
// 🟢 コールバック関数の例（メイン関数内の追加処理 vs コールバック関数内の追加処理）
// =======================================
...
function a(b) {
    ...
    console.log("【1】Hi,Cat!（メイン処理）");

    // 👉 コールバックを呼び出す（ここで外部からの処理が差し込まれる）
    b(); 

    // 👉 メイン関数内での追加処理
    console.log("【2】追加処理：これはメイン関数の一部なので必ず実行される");
    // 💡 固定的（毎回同じ）→ 先生の「授業まとめ」のような役割
}
...
function c() {
    // 👉 コールバック関数内での追加処理
    console.log("【3】Hi,Bat!（コールバック処理）");
    console.log("【4】Hi,Dog!（さらに追加可能：差し替え自由）");
    // 💡 柔軟に差し替え可能 → 生徒が「何を発表するか」を自由に決めるイメージ
}
...
// b() が実行 → c() が呼ばれる 🚩STEP3
//       ↓
// 【3】"Hi,Bat!" が出力（コールバック処理）
// 【4】"Hi,Dog!" が出力（コールバック処理差し替え可能）
//       ↓
// 【2】メイン関数内の追加処理も実行（必ず実行される）
//       ↓
// 完了！ 🎉
