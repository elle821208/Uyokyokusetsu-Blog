...
///<!--2:02:19～-->
//querySelector
console.log(" ↓ querySelectorで一つのid #foofooを取得した。idは一つの変数としてカウントされる。htmlにおいてidは1ページに一つしか存在できない。");
console.log(document.querySelector("#foofoo"));//idは#をつける。一つの変数としてカウントされる。htmlにおいてidは1ページに一つしか存在できない。
console.log(" ↓ querySelectorAllで複数のclass .barbarを配列として取得した。classは複数の配列としてカウントされる。htmlにおいてclassは1ページに複数存在できる。");
console.log(document.querySelectorAll(".barbar"));//classは.をつける。classを全て取得する。配列になる。
console.log(" ↓ []で配列の一つ目を指定し、innerHTMLで編集する。");
