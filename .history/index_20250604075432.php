        ...
        <div class="articles_container"><!--サブループその１＆その２（技術ブログ用）-->
            ...
            <div class="grid-articles"><!--← グリッドデザイン（サブループその１） -->
                ...
                $tech_posts = new WP_Query(array(
                    // $tech_posts という変数を作って、そこに検索結果を入れる
                    // new は「新しく作る」という意味
                    // WP_Query は「投稿を検索するためのクラス（道具）」
                    // array の中で「どんな投稿を探すか（条件）」を指定する
                    'category_name' => 'tech', // 「tech」カテゴリの投稿を
                    'posts_per_page' => 5      // 最大5件まで表示する
                ));
                ...
                if ($tech_posts->have_posts()) : // have_posts() で投稿がまだ残っているかチェックする。もし投稿が存在する場合（取得できたら）に以下を実行
                    while ($tech_posts->have_posts()) : // while は「〜の間ずっと」という意味のくり返し文。取得した投稿がある間、1件ずつ処理を繰り返す
                        $tech_posts->the_post(); // 投稿を取り出して表示できるように準備する。これをしないと、the_title() や the_content() は使えない
                ?>
            ...
            <!---->
            <!--カスタム投稿タイプ tech（「functions.php」で登録した新規の投稿一覧） を「index.php」でサブループを使って表示するためのコード-->
            <!-- サブループに入れる条件は配列$argsに入れている。 -->
            <?php
            // 投稿の条件をまとめた「$args（引数）」という変数を作る
            $args = array(
                // 'post_type' は「どの投稿タイプを探すか」
                // 今回は functions.php で作った「tech」という投稿タイプを探す
                'post_type' => 'tech',

                // 'posts_per_page' は「1ページに何件の投稿を表示するか」
                // 今回は最大5件まで表示する（すべて表示したいときは -1 にする）
                'posts_per_page' => 5,

                // 'paged' は「今何ページ目なのか？」を指定する（ページ分け対応）
                // 例えば、2ページ目なら2、3ページ目なら3が入る
                'paged' => $paged
            );

            // WP_Query という「投稿を検索する道具（クラス）」を使って
            // 条件（$args）に合う投稿を探し、その結果を $the_query に入れる
            $the_query = new WP_Query($args);

            // もし投稿が見つかったら（あれば）、表示の準備を始める
