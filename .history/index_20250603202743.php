    ...
    <section id="information">
        ...
        <div class="articles_container"><!--サブループその１＆その２（技術ブログ用）-->
            ...
            <div class="grid-articles"><!--← グリッドデザイン（サブループその１） -->
                <?php//WordPressの投稿の中から、カテゴリが「tech」のものを、最大5件だけ探して、それを $tech_posts に保存する。


                $tech_posts = new WP_Query(array(// WP_Query は WordPress がもともと用意してくれている「投稿を検索するための便利な道具（クラス）」。new WP_Query(...) は、「その道具を新しく使う」という意味。
                    //arrayでこの中で「どんな投稿を探したいか」を指定する
                    'category_name' => 'tech', // ← tech」カテゴリの投稿を
                    'posts_per_page' => 5// 最大5件まで表示して
                ));
                if ($tech_posts->have_posts()) :// もし投稿が存在する場合（取得できたら）に以下を実行
                    while ($tech_posts->have_posts()) : // 取得した投稿がある間、1件ずつ処理を繰り返す
                        $tech_posts->the_post();// the_postで「1件の投稿を、WordPressのテンプレートタグ（例えば the_title() や the_content()）で使えるように準備する」ための命令
                ?>
