...
// =======================================
// wordpressからの読み込みテスト(functions.js)
// =======================================
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (box) {
    ...
    box.innerText = "JavaScript(js/Tetsu-Js-Study/functions.js)から書き換えました！";
  }
});
...
// ======= セクション1：通常の関数 =======
console.log("🚩【セクション1】通常の関数宣言と引数の流れ");

// ① 関数定義
function introduce(name, job) {
    // ③ 関数内で引数を利用して処理
    console.log("私は " + name + " です。"); // name に "哲也" が渡る
    console.log(job + " です。");           // job に "プログラマー" が渡る
}
...
- 関数は「呼び出さないと実行されない」
- 第一引数 "哲也" が name に渡る
- 第二引数 "プログラマー" が job に渡る
*/


// ======= セクション2：関数式（無名関数） =======
console.log("🚩【セクション2】関数式と引数の流れ");

// ① 関数式の定義（無名関数を変数に代入）
const introduce2 = function (name, job) {
    console.log("私は " + name + " です。"); // name に "tetsuya" が渡る
    console.log(job + " です。");           // job に "コーダー" が渡る
};
...
/*
補足：
- introduce2 という変数に関数を代入している
- 関数を「値」として扱える（柔軟性が高い）
*/


// ======= セクション3：function と 関数式の違い =======

/*
【function 宣言の特徴】
✅ コードが読みやすい（名前付き）
✅ ホイスティングされるので、定義より前でも呼び出せる
❌ 柔軟性は低い（変数に代入できない）

【関数式（無名関数）の特徴】
✅ 変数に代入できる → 関数を値として扱える
✅ 柔軟に作れる（一時的な関数など）
❌ ホイスティングされない（定義より前に呼ぶとエラー）

👉 実務では「基本は function」「必要なときに関数式」を使う
*/
