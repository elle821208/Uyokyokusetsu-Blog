...
// (1) グローバル変数とは？
// =======================================
// 「グローバル変数」とは、プログラム全体（どこからでも）使える変数のこと。
// 例えるなら「学校全体でのルール」みたいなもの。

const globalNumber = 5; // (1-1) プログラム全体で使える変数globalNumberを作り、値「5」を入れる。

// =======================================
// (2) 関数 showLocalNumber を作る
// =======================================
// 「関数」とは、やりたいことをまとめた「手順書」や「レシピ」のようなもの。

function showLocalNumber() {  
  // (2-1) ここから「関数 showLocalNumber の中」の世界。
  // 関数の中で作る変数は「ローカル変数」。
  // 「ローカル変数」はその関数の中でしか使えない。
  // 学校で言うと「この教室の中だけのルール」。

  const localNumber = 777; // (2-2) 関数の中だけで使える変数localNumberを作り、「777」を入れる。

  console.log("関数 showLocalNumber の中でつかえる変数function showLocalNumber" + localNumber); // (2-3) ここではlocalNumberの値「777」が表示される。
}  
// (2-4) ここまでが関数 showLocalNumber の中の世界。

showLocalNumber();  
// (2-5) 上で作った関数 showLocalNumber を「実行」する。
// 実行すると、localNumber = 777 が表示される。

// =======================================
// (3) グローバル変数をもう一度確認
// =======================================
console.log(globalNumber);
// (3-1) ここで表示されるのは、関数の中のlocalNumberではなく、最初に作った「globalNumber」。
// (3-2) だから、「5」が表示される。
