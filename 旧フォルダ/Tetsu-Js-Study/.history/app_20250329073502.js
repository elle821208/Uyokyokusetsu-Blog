...
if (isLogin === false) {
  ...
  console("isLoginさんはfalseなのでログイン1してください"); //左を実行する。alertで警告文を出す。
}
...
//グローバル変数<!--1:29:00～-->
let barbar =6;//変数barbarを定義する。値は6。※ここはスコープ（関数jooの中）の外側。

function joo() {//関数joo
    // const barbar = 5;//スコープ内（関数jooの中）で変数barbarを定義している。
    
    console.log(barbar);//で変数barbarを呼び出す。5が表示される。
}

// console.log(barbar);//スコープ外。変数bar6を呼び出す。6が表示される※ここは関数jooの外側。
