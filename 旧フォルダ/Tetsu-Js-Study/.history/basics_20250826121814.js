...
console.log("🎯 ↑↑↑ 今回の初期設定 → hasTicket = true（チケットあり）, isAdult = false（子供）");

// 「チケットを持っていて、なおかつ大人なら入場できる」
if (hasTicket && isAdult) {
  console.log("🎫 チケットもあって大人なので入場できます！ ✅");
  console.log(" ← Mosi初期設定で hasTicket と isAdult が両方 true なので、このメッセージが表示されます。");
} else {
  console.log("🚪 チケットがないか、子供なので入場できません… ❌");
  console.log(" ← もしも初期設定で hasTicket と isAdult が両方 false またはどちらかが false ならば、このメッセージが表示されます。");
}
...
if (ateMeal || ateSnack) {
  ...
  console.log(" ← 今回の初期設定では ateMeal = false, ateSnack = true なので、このメッセージが表示されます。");
} else {
  console.log("🍴 何も食べてないのでお腹が空いた… 🥺 ❌");
  console.log(" ← もしも初期設定で ateMeal と ateSnack が両方 false ならば、このメッセージが表示されます。");
}
