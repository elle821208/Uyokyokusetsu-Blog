...
// 【例1】判定する関数 コールバック関数_judgeFunction
function コールバック関数_judgeFunction(コールバック引数_n) {
    // コールバック引数_n は数字（ここでは 10 や 3 が渡される）

    // 例：骨組み関数_skeletonFunction(10, コールバック関数_judgeFunction) とすると
    // コールバック引数_n には 10 が入るので「10 > 5」を判定することになる。

    if (コールバック引数_n > 5) {
        console.log("判定関数 → 大きい！（今回は コールバック引数_n が 5 より大きいから）");
    } else {
...
// 【例2】計算する関数 コールバック関数_doubleFunction
function コールバック関数_doubleFunction(コールバック引数_n) {
    // コールバック引数_n = 数字（ここでは 骨組み関数_skeletonFunction から渡される値、例: 5）

    // 【説明】
    // コールバック関数_doubleFunction は 骨組み関数_skeletonFunction に渡される「コールバック関数」の一例
    // 骨組み関数_skeletonFunction は
    // 「どう処理するかは知らない、ただ渡された関数を呼び出すだけ」
    // なので、コールバック関数_doubleFunction は「数字を2倍にする処理」を自由に行える

    // 例：
    // 骨組み関数_skeletonFunction(5, コールバック関数_doubleFunction) と呼び出すと、
    // - コールバック引数_n = 5 が コールバック関数_doubleFunction に渡される
    // - コールバック引数_n * 2 = 10 の計算結果を出力する
    // → コンソールには "計算関数 → 2倍 = 10" と表示される

    console.log("計算関数 → 2倍 =", コールバック引数_n * 2);
}
...
// =======================================
// - コールバック関数_judgeFunction：
//    入力された数字を「5より大きいかどうか判定」して結果を出力する処理
//    （条件分岐を使った判定型の処理）
//
// - コールバック関数_doubleFunction：
//    入力された数字を「2倍にして出力」する処理
//    （計算に特化した処理）
//
// 👉 どちらも 骨組み関数_skeletonFunction に渡せるが、
