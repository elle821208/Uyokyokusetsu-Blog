...
const metalBurst = (lastDamage) => {//アロー関数式metalBurst。引数lastDamageに呼び出し元の値が入る。
    return lastDamage*1.5;//関数の実行内容。戻り値は"引数の値*1.5"。
}
console.log(metalBurst(50));//（12）呼び出し元。関数metalBurstを呼び出す。引数の値は仮で"50"としてみた。
