...
console.log(
  "ここがconsole呼び出し元。右記のmetalBurst(50)の引数を50に設定。上記の関数 metalBurst()に引数が渡った後、関数の中にある戻り値 lastDamage * 1.5 = 75 を関数の外に返す。→ " + metalBurst(50)
);
// (2-1) 関数 metalBurst を「50」という引数を渡して実行。
// (2-2) 実行すると、関数の中で 50 * 1.5 の計算が行われ、戻り値は 75 になる。
// (2-3) その結果を console.log で表示している。
// ➜ 結果：「呼び出し元の引数を50に設定。上記の関数 metalBurst の中にある戻り値 lastDamage * 1.5 = 75 → 75」が出力される。
