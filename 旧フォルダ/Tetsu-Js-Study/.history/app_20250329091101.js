...
//
function introduce(name, job) {//関数introduce 引数(関数に与える情報)name、job
    console.log(`私は${name}です。`)//関数introduceの処理内容。コンソールで出力する。
    console.log(`${job}です。`)//同上
}

introduce('ta1fukumoto', 'エンジニア');//上記で設定した関数introduceを実行、今回はコンソールで出力する。あらかじめ引数の値も設定しておく。
//関数introduceを実行すると設定した引数の値('ta1fukumoto', 'エンジニア')が関数introduceのname、jobに値が渡される。
//下記が表示される。
//私はta1fukumotoです。
//エンジニアです。

introduce('Messi', 'フットボールプレイヤー');//上記の関数introduceを実行、今回はコンソールで出力する
//私はMessiです。
//フットボールプレイヤーです。
