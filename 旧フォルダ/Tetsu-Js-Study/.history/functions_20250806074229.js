...
// 名前を表示する関数
function showName() {
  console.log("追加処理（その１） → Hi,Bob!");
}

// 年齢を表示する関数
function showAge() {
  console.log("追加処理（その２） → I'm 30 years old!");
}

// ↓↓↓ 呼び出し元
// それぞれの関数をコールバックとして渡す
console.log("=== 呼び出し（その１） → 名前紹介 ===");
greet(showName);

console.log("=== 呼び出し元（その２）→ 年齢紹介 ===");
greet(showAge);
...
function sayGuestName() {
  ...
  console.log("対追加処理（その１） → Hell,hello,Ken!");
  // (6) ← 「Hi,Ken!」と表示して終了
}

// ▼ 追加関数②：ゲストの年齢を紹介
function sayGuestAge() {
  console.log("追加処理（その２） → I'm,mmmmm 25 years old!");
}
