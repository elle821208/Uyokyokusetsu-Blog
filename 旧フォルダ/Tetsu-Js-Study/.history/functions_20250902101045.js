...
console.log("↓↓↓ a = メイン関数（固定の骨組み）、b = コールバック引数、c = コールバック関数（差し込める処理） ↓↓↓");

// =======================================
// 🚩STEP2: メイン関数定義 a（呼び出される側）
// =======================================
function a(b) {
    // 【説明1】
    // 呼び出し側に a(c); と呼び出されたとき、ここの メイン関数 a(b) の b に c が渡される
    // よって「b = c」となる（この仕組みがコールバックの基本）

    console.log("【1】メイン処理：必ず実行される基本の動作（骨組み部分）");

    // 【説明2】
    // メイン関数の中でも「追加処理」を書ける
    // これは必ず実行される（差し替えできない固定の処理）
    console.log("【2】メイン関数の追加処理：毎回必ず実行される（まとめや後片付けのイメージ）");

    // 【説明3】
    // b は c の「別名」になっている
    // 今回は「メイン処理がすべて終わったあと」にコールバックを呼び出す
    // こうすることで「完了通知」や「まとめ処理」として使える
    // 💡 コールバック関数はこのように「最後に呼ぶ」こともできるし、
    //    必要に応じて「メイン処理の途中で呼ぶ」こともできる（柔軟に配置できるのが特徴）
    console.log("👉 最後にコールバック関数 c() を呼び出すよ！ ← 呼び出し側a(c);の引数");
    b(); 
}

// =======================================
// 🚩STEP3: コールバック関数 c（差し込まれる処理、呼び出し側a(c);の引数）
// =======================================
function c() {
    // 【説明4】
    // ここからは呼び出し側が自由に決められる処理
    // 必要に応じて内容を差し替え可能
    console.log("【3】コールバック処理：差し込まれた具体的な動作、呼び出し側a(c);の引数（自由に変更できる部分）");

    // 【説明5】
    // さらに追加で処理を入れることも可能
    // 例: 別のメッセージを出す、別の動作をする、など
    console.log("【4】コールバック処理の追加：自由に差し替えられるアクション");
}

// =======================================
// 🚩STEP1: 呼び出し（実行開始、人間はここから理解する）
// =======================================
a(c); // 【説明6】関数呼び出しスタート（a に c を渡す）
...
// 呼び出し: a(c) 🚩STEP1
//       ↓
// b = c   ← a(c) = a(b) よって b は c を指す 🚩STEP2
//       ↓
// 【1】メイン処理が実行（必ず実行される骨組み部分）
//       ↓
// b() が実行される → c() が呼ばれる 🚩STEP3
//       ↓
// 【3】コールバック処理（差し込み自由）
// 【4】コールバック処理の追加（差し替え可能）
//       ↓
// 【2】メイン関数の追加処理（固定で必ず実行される）
//       ↓
// 完了！ 🎉
...
// =======================================
// 🟢 if文入り関数との違い・メリット・デメリット
// =======================================

// if文を関数に入れる場合の例
function checkNumber(num) {
    if (num > 5) {                     // ← 条件チェック（自分で判断）
        console.log("大きいよ！");
    } else if (num > 2) {
        console.log("ちょっと大きめ");
    } else {
...
// 呼び出し
checkNumber(10); // → 大きいよ！
checkNumber(4);  // → ちょっと大きめ
checkNumber(1);  // → 小さいよ！

// デメリットを強調:
// ・動き方を変えたい場合、if文や関数の中身を書き換えなければならない → 呼び出す側は自由に追加処理できない
// ・条件が増えるとどんどんコードが長くなる → 読みにくくなる
// ・固定されたルールしか使えず、差し替えや再利用がしにくい

// コールバック関数の場合
// ・メイン関数が外から渡された関数を実行する仕組み
// ・メリットを強調:
//   → 同じメイン関数でも追加処理を自由に差し替え可能（複数の追加処理もまとめられる）
//   → イベント処理や動作のカスタマイズが簡単
//   → if文より柔軟で拡張性が高い

// まとめ:
// - if文入り関数 = 自分で考えるキャラ（動きは固定、書き換えが必要）
// - コールバック関数 = 外から指示を受けるキャラ（動きを自由に差し替え可能、柔軟性抜群）
