...
///<!--2:08:03～-->
    ...
    const $post = document.createElement("article");//変数$postを宣言。createElementでHTMLタグを生成する。（今回は引数どおりのarticleタグを生成する。）を生成する。
    $post.setAttribute("class", "post");//変数$post。"post"（第二引数）という"クラス"（第一引数）を持った"articleタグ"（上記の）を生成する。
    $post.innerText = "お腹減ったなう。";//変数$post。
