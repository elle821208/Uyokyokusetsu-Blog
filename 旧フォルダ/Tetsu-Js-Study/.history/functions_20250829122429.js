...
function getSeason(month) {
    // 作業の流れ:
    //  (1) 下記 const season の右辺で currentMonth の値を引数として受け取る
    //  (2) 関数内で if 文により季節を判定
    //  (3) 判定結果が戻り値として返される
...
//  (2) .getMonth() で「月（0～11）」を取得
//  (3) +1 して「1～12」の形式に変換し currentMonth に代入
console.log(`2️⃣ 現在の月 (currentMonth): ${currentMonth} （今回の値: 8）`);

// 3. 関数を呼び出して季節を取得（season は const 定数）
const season = getSeason(currentMonth);
// 作業の流れ:
//  (1) currentMonth の値（例: 8）を引数として関数 getSeason(month) に渡す
//  (2) if 文で季節が判定され、戻り値が season に代入される
...
// 3. 値を整形（+1）して変数に代入
// 4. const season の右辺で関数呼び出し → 引数に値を渡す
