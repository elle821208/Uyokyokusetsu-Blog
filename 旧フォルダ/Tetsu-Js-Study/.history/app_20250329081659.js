...
//グローバル変数<!--1:29:00～-->
const bard = 5;//変数を定義する。値は6。※ここはグローバル変数。スコープ（関数jooの中）の外側。

function koo() {//関数joo
    const bard = 6;//ここはローカル変数。スコープ内（関数jooの中）で変数を定義している。
    console.log(bard);//でローカル変数を呼び出す。表示されない？
}

console.log(bard);//スコープ外。グローバル変数を呼び出す。6が表示される※ここは関数jooの外側。
...
function joo() {
    const bardy = 66;
    console.log(bardy); // 66が表示される
}

joo(); // 関数を実行
console.log(bardy); // 55が表示される
