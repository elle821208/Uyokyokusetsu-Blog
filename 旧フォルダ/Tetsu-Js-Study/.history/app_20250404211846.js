...
const pikachu2 = {
  ...
  //メソッドlevelUpを実行すると
  //levelが1ずつ増えて、
  //配列を作って、取り出して、増減式で１ずつ増やす。２０でskillsにスパークを追加する。
  //↓↓↓{}の中の処理にif文を置き繰り返し処理の値を作る。
  ////配列arの変数iの数(繰り返した数)だけ呼び出す。
  ////初期値：変数iで最初の配列の数は0、条件式iが配列arの配列の数より小さい時、増減式i++で配列の数iが1ずつ増ええていく
  levelUp: function () {
      for (let i = 0; i < 1; i++) {//真ん中の条件式(I < 1)のために変数iは1回だけループする。
          this.value2++;//pikachu2.levelUp(オブジェクトのプロパティ)
