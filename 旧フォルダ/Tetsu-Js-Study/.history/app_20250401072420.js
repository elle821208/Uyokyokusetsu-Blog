...
//１～６のいずれかの整数をランダムに返す
const loo = (1 + Math.floor(Math.random() *6));
console.log(loo);
// いくつかのカッコが組み合わさっていますが、その一番内側から見ていきます。
// Math.random()*6
// ６種類の結果が欲しいので、Math.random()に6を掛けています。
...
// と、単純に１を足すだけで、「1か2か3か4か5か6」のどれかをランダムで返す「サイコロ」になります^^




//１～６のいずれかの整数をランダムに返す
const moo = (1+Math.floor(Math.random()*6));
console.log(moo);
//いくつかのカッコが組み合わさっていますが、その一番内側から見ていきます。
// Math.random()*6/６種類の結果が欲しいので、Math.random()に6を掛けています。
//すると、この部分の数値は「0～5.999999…」の範囲内の、いずれかの数値、という事になります。（この時点では小数のある数値なので膨大なパターンの結果になりえます）
//そしてそれを、 Math.floor(Math.random()*6)…と、Math.floor関数の引数として与える事で、「0か1か2か3か4か5」のどれか、という数値になります。
//結果として欲しいのは「1～6」なので、1+Math.floor(Math.random()*6)と、単純に１を足すだけで、「1か2か3か4か5か6」のどれかをランダムで返す「サイコロ」になります^^


//12％の確率で「当たり」
const noo =(Math.floor(Math.random()+0.12));
console.log(noo);
// 設定したい確率を、小数として Math.random() に足します。
// 今回の例は12%なので、0.12を足しています。
// また内側のカッコの内容から見ていきます。
// Math.random()+0.12
// Math.random()は「0～0.999999…」の範囲でしたが、それに0.12を足した物は、「0.12～1.119999999…」(←0.12を足した)の範囲から、ランダムで数値が返ってくる事になります。

// Math.random()は「0～0.999999…」の範囲でしたが、それに0.30を足した物は、「0.30～1.299999999…」(←0.12を足した)の範囲から、ランダムで数値が返ってくる事になります。
