...
const hornDrill = (theirHp) => {//関数hornDrill 引数theirHp を設定する。
...
console.log(hornDrill(100) + "←関数hornDrillの実行結果ですよ");//関数hornDrillの引数theirHpの値は仮で100とする。
...
const snsUser2 = {
    id: 1,
    userName: "Tomoko",
    post: function (contents) {//メソッドpost。()内に引数contents。{}内に処理内容。
        return contents + "を投稿しました by…";//戻り値は引数contentsの値"プログラミングなう。"
    }
};

console.log(snsUser2.post("プログラミングなう。"));//呼び出し元。snsUser2のメソッドpostを呼び出す。
// ()内の"プログラミングなう。"はメソッドpostの引数contentsの値。
