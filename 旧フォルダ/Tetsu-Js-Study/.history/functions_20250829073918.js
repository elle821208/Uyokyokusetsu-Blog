...
// 例: new Date() → Thu Aug 28 2025 21:00:00 GMT+0900 (Japan Standard Time)
//
// 【オブジェクトとメソッドの違い】
// - オブジェクト: データそのもの（例: new Date() = 現在日時のかたまり）
// - メソッド: オブジェクトに付属する関数（例: .getHours(), .getMonth()）
//   → オブジェクトから「特定の情報」を取り出す処理
//
// 例: 
//   const now = new Date();      ← now はオブジェクト（日時そのもの）
//   now.getHours();              ← メソッド呼び出し（時間だけ取り出す）
//   now.getMonth();              ← メソッド呼び出し（月だけ取り出す）
...
function getDarkModeStatus(time) {
    // 💡 作業の流れ 1: getDarkModeStatus() に引数が渡される
    //   const mode = getDarkModeStatus(new Date().getHours()); ← 呼び出し側
    //   ↓
    //   new Date().getHours() の値（例: 21）が time に渡される
    //   ↓
    //   function getDarkModeStatus(time = 21) { ... } となるイメージ
    console.log(`1️⃣ 引数 time に渡された値: ${time} （今回の値: 21）`);

    // 💡 作業の流れ 2: time の値に応じて判定
    if (time > 20) {
        return "dark";
    } else {
        return "light";
...
// 💡 作業の流れ 3: new Date() → 現在の日時を生成（オブジェクト）
// 💡 getHours() → new Date() から「時間」を取り出すメソッド
// 💡 2025年8月28日 21時なので → 21 が返る
console.log(`2️⃣ new Date().getHours() の値: ${new Date().getHours()} （今回の値: 21）`);
console.log(`2️⃣ 関数 getDarkModeStatus に渡された time の値: ${mode === "dark" ? 21 : 0} （今回の値: 21）`);

// 3. 背景色を変更する処理
// 👀 ユーザー視点: ここから「動作している」と感じる部分（画面の見た目が変わる）
if (mode === "dark") {
...
// 4. 結果をコンソールに表示
// 👀 ユーザー視点: コンソールで現在のモードを確認できる
console.log(`3️⃣ 現在のモード: ${mode} （今回の値: dark）`);

// =======================================
// まとめ（背景色変更）
// =======================================
// 【開発者視点の流れ】
// 1️⃣ new Date().getHours() で現在の時間（0～23）を取得 → 今回は 21
// 2️⃣ getDarkModeStatus(time) を呼び出す → getDarkModeStatus(21)
// 3️⃣ 引数 time に 21 が渡され、if 文で判定
// 4️⃣ time > 20 なので "dark" が返される
// 5️⃣ 戻り値 "dark" が mode に代入され、背景色変更や出力に利用される
//
// 【ユーザー視点の動作開始】
// 👉 3. 背景色を変更する処理 から
// 👉 画面の色が変わり、コンソールに「現在のモード: dark」と表示されて初めて動いたと感じる
...
function getSeason(month) {
    // 💡 作業の流れ 1: getSeason(currentMonth) に引数が渡される
    //   const season = getSeason(currentMonth); ← 呼び出し側
    //   ↓
    //   currentMonth の値（例: 8）が month に渡される
    //   ↓
    //   function getSeason(month = 8) { ... } となるイメージ
    console.log(`1️⃣ 引数 month に渡された値: ${month} （今回の値: 8）`);

    // 💡 作業の流れ 2: month の値に応じて季節を判定
    if (month >= 3 && month <= 5) {
        return "春";
...
// 💡 作業の流れ 3: new Date() → 現在の日時を生成（オブジェクト）
// 💡 getMonth() → new Date() から「月」を取り出すメソッド（0始まりで返る）
// 💡 +1 することで 1～12 月の形式に変換
// 💡 2025年8月28日現在: getMonth() = 7 → +1 → currentMonth = 8
console.log(`2️⃣ 現在の月 (currentMonth): ${currentMonth} （今回の値: 8）`);

// 3. 関数を呼び出して季節を取得（season は const 定数）
// 👀 ユーザー視点: ここから「季節判定の動作が始まった」と感じる
const season = getSeason(currentMonth);
console.log(`3️⃣ 関数 getSeason に currentMonth (${currentMonth}) を渡した結果: ${season} （今回の値: 夏）`);

// 4. 結果をコンソールに表示
// 👀 ユーザー視点: 季節の判定結果がコンソールに表示される部分
console.log(`4️⃣ 現在の月は ${currentMonth} 月です （今回の値: 8）`);
console.log(`4️⃣ 🌟 なので、季節は ${season} です！ （今回の値: 夏）`);

// =======================================
// まとめ（季節判定）
// =======================================
// 【開発者視点の流れ】
// 1️⃣ currentMonth に「今の月（1～12）」が入る → 今回は 8
// 2️⃣ getSeason(currentMonth) を呼び出す → getSeason(8)
// 3️⃣ 引数 month に currentMonth（8）が渡される
// 4️⃣ month の値に応じて if 文で季節を判定 → 夏
// 5️⃣ 戻り値「夏」が season に代入され、表示される
//
// 【ユーザー視点の動作開始】
// 👉 3. 関数を呼び出して季節を取得 から
// 👉 コンソールに「現在の月は 8 月です」「季節は 夏です！」と出て初めて動いたと感じる
