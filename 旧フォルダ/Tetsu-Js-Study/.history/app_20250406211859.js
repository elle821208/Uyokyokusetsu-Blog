...
console.log(" ↓ 配列になった.barbarクラスから[0]で配列の一つ目を指定し、innerHTMLで編集する。");
console.log(document.querySelectorAll(".barbar")[0].innerHTML = "<div>配列としてまとめた.barbarクラスの一つ目を指定し、innerHTMLで編集する</div>");
console.log(document.querySelectorAll(".barbar")[1].innerHTML = "<div>配列としてまとめた.barbarクラスの一つ目を指定し、innerHTMLで編集する</div>"  );

console.log(document.getElementById("foofoo"));//id。一つの変数としてカウントされる。
console.log(document.getElementsByClassName("barbar"));//classを全て取得する。配列になる。
