...
console.log(
  (document.querySelector("#foofoo").innerHTML =
    "← jsのdocument.querySelector(#foofoo).innerHTMLを使う。HTMLの1つのidをquerySelectorでひとつの変数として取得した。htmlにおいてidは1ページに一つしか存在できない。")
);
// 🧠 ポイント：
...
console.log(
  (document.querySelectorAll(".barbar")[0].innerHTML =
    "← jsのdocument.querySelectorALL(barbar)[0].innerHTMLを使う。HTMLの4つのbarbarクラスをquerySelectorAllで配列としてまとめた。これは[0]番目のBg-Aquaを呼び出した。")
);
// 🧠 ポイント：
...
console.log(
  (document.querySelectorAll(".barbar")[1].innerHTML =
    "<div>.barbarクラスの配列の中身を[番号]で指定し、innerHTMLで編集する。これは[1]番目。※※querySelectorAll（idもクラスも取得可）で取得した。</div>")
);
