...
function introduce(name, job) {
  //functionで関数introduceを定義し、 引数(関数に与える情報)name、job
    console.log(`ここが呼び出し元。上記の関数functon introduceに、ここと下記の引数の値が入り、
...
const tetsu = 18000 + 10000; //const変数tetsuに値5+1を代入する。
console.log("↓↓↓コールバック関数（例１）" + tetsu); //変数fooを出力する。コンソールで28000が表示される。

// メイン関数：あいさつのあとに追加処理（コールバック関数）を実行する
function greet(cb) {
  console.log("メイン処理 → Hi,Amy!"); // メインのあいさつ
  cb(); // コールバック関数を実行
}
