...
console.log(" ↓ querySelectorAllで複数ある.barbarクラスを配列としてまとめて取得した。htmlにおいてclassは1ページに複数存在できる。");
console.log(document.querySelectorAll(".barbar").innerHTML = "<div>querySelectorAllで複数ある.barbarクラスを配列としてまとめて取得した。htmlにおいてclassは1ページに複数存在できる。</div>");//classは.をつける。classを全て取得する。配列になる。
console.log(" ↓ 配列になった.barbarクラスから[]で配列の一つ目、二つ目を指定し、innerHTMLで編集する。");
console.log(document.querySelectorAll(".barbar")[0].innerHTML = "<div>配列としてまとめた.barbarクラスの一つ目を指定し、innerHTMLで編集する</div>");
console.log(document.querySelectorAll(".barbar")[1].innerHTML = "<div>二つ目も同上</div>");

console.log(document.getElementById("foofoo") );//id。一つの変数としてカウントされる。
console.log(document.getElementsByClassName("barbar")[2].innerHTML = "<div>三つ目も同上</div>");//classを全て取得する。配列になる。
