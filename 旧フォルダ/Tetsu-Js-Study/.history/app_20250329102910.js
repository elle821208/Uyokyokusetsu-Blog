...
//
//戻り値として面積を計算した値を返す関数(アロー関数と関数式)
const area = (height, width) => {//アロー関数式area(引数を設定)
    return height * width;//アロー関数areaの戻り値(関数areaの実行内容)
};

console.log(area(12, 5));//コンソールで関数areaを出力する際にアロー関数areaの戻り値(関数areaの実行内容)が。引数の値は12、5を設定しておく。
//60
