...
function getDarkModeStatus(time) {
    // 👉 補足説明
    console.log(`引数 time に渡された値: ${time}（今回の値: ${time}）`);
    console.log("👉 time は getDarkModeStatus() を呼び出すときに渡された値");
    console.log("👉 今回は new Date().getHours() の結果（現在の時間 0～23）を受け取っている");
    console.log("   例えば、現在が 21 時なら new Date().getHours() は 21 を返す → その 21 が time に入る");
    console.log("   もし現在が 10 時なら new Date().getHours() は 10 を返す → その 10 が time に入る");
...
// 👉 補足説明
console.log("👉 new Date() は『現在の日時』を生成するオブジェクト");
console.log(`👉 new Date() の値: ${new Date()}（今回の値: ${new Date()}）`);
console.log("👉 getHours() は 0～23 の整数で『現在の時間』を返すメソッド");
console.log(`👉 new Date().getHours() の値（現在の時間）: ${new Date().getHours()}（今回の値: ${new Date().getHours()}）`);
console.log("👉 この new Date().getHours() の戻り値（例: 21 や 10 など）が、そのまま関数 getDarkModeStatus(time) の引数 time に渡されている");
...
// 👉 補足説明
console.log(`現在のモード: ${mode}（今回の値: ${mode}）`);
console.log("👉 getDarkModeStatus(time) 関数は、時間を渡すと背景モードを返す（dark or light）");
console.log("👉 関数は戻り値を使うことで、他の処理でも再利用できる");
...
function getSeason(month) {
    // 👉 補足説明
    console.log(`引数 month に渡された値: ${month}（今回の値: ${month}）`);
    console.log("👉 month は getSeason() を呼び出すときに渡された値");
    console.log("👉 今回は currentMonth（new Date().getMonth() + 1 の結果）が month に渡されている");
    console.log("👉 処理の流れを整理すると：");
    console.log("   1. const season = getSeason(currentMonth); が呼ばれる");
    console.log("   2. そのとき currentMonth の値（例: 4）が関数に引数として渡される");
    console.log("   3. function getSeason(month) { ... } の (month) の部分にその値が入る");
    console.log("   4. つまり『getSeason(currentMonth) → getSeason(4) → function getSeason(month=4) { ... }』という流れになる");
    console.log("👉 こうして関数の中で month が使えるようになり、その month をもとに季節を判定している");
...
console.log("👉 new Date() は『現在の日時』を生成するオブジェクト");
console.log(`👉 new Date() の値: ${new Date()}（今回の値: ${new Date()}）`);
console.log("👉 getMonth() は『現在の月』を返すメソッド。ただし 0 始まり（1月→0, 2月→1, ... 12月→11）");
console.log(`👉 new Date().getMonth() の値（今回の月-1）: ${new Date().getMonth()}（今回の値: ${new Date().getMonth()}）`);
console.log("👉 そのため +1 をすることで、人間が普段使う 1～12 の月に直している");
console.log(`👉 最終的に currentMonth に入る値: ${currentMonth}（今回の値: ${currentMonth}）`);

// 3. 関数を呼び出して季節を取得
const season = getSeason(currentMonth);

// 4. 結果をコンソールに表示
console.log(`現在の月は ${currentMonth} 月です（今回の値: ${currentMonth}）`);
console.log(`🌟 なので、季節は ${season} です！（今回の値: ${season}）`);

// 👉 補足説明
console.log("👉 getSeason(month) 関数は、月を渡すと季節名を返す関数です");
console.log("👉 戻り値を使うことで、コンソール出力だけでなく他の処理にも再利用可能です");
