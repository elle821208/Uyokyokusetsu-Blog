...
// === は「左と右がまったく同じか」を調べる
if (isLogin1 === false) { // isLogin1 が false のとき実行
  console.log("🚪 ログインしてください ❌");
  console.log(" ← 変数 isLogin1 の値が false なので、この if 文が実行されました。");
  console.log(`条件式 "isLogin1 === false" が true になったため、{} 内の処理（console.log）が動きました。`);
}

/*
【補足】
・if (isLogin1 === false) は「もし isLogin1 が false なら…」という意味
・=== は省略できる場合もある
   例）if (isLogin1 === true) → if (isLogin1)
       if (isLogin1 === false) → if (!isLogin1)
...
let isLogin3 = true; 
// 変数 isLogin3 にログイン状態を代入する
// true = ログイン済み、false = 未ログイン

// if(条件式) { ... } は「もし条件がOKなら…する」
// !（びっくりマーク）は「反対」にするスイッチ
// つまり !isLogin3 は「isLogin3 が false の場合と同じ意味」
if (!isLogin3) { // isLogin3 が false のとき実行
  console.log("🚪 ログインできていません ❌");
  console.log(" ← 変数 isLogin3 の値が true ではないため、この if 文が実行されました。");
} else { // それ以外（true）のとき実行
  console.log("✅ ログイン済みです！ようこそ 🎉");
  console.log(" ← 変数 isLogin3 の値が true なので else 文が実行されました。");
  console.log(`if 条件式の "!isLogin3" は "isLogin3 が false" と同じ意味です。今回は false ではないため、else に入りました。`);
}

/*
【動作シミュレーション表】

| isLogin3 の値 | !isLogin3（条件） | 実行されるブロック | コンソール表示                         |
|---------------|-------------------|--------------------|----------------------------------------|
...
// ---------- &&（かつ）の例 ----------
let hasTicket = true;   // チケットを持っているか（true = 持っている）
let isAdult = false;    // 大人かどうか（true = 大人、false = 子供）

// 「チケットを持っていて、なおかつ大人なら入場できる」
if (hasTicket && isAdult) { // 両方 true なら実行
  console.log("🎫 チケットもあって大人なので入場できます！ ✅");
  console.log(` ← hasTicket = ${hasTicket} かつ isAdult = ${isAdult} が両方 true なのでOK。`);
} else {
  console.log("🚪 チケットがないか、子供なので入場できません… ❌");
  console.log(` ↑↑↑ hasTicket = ${hasTicket} または isAdult = ${isAdult} が false なのでNG。`);
}

// ---------- ||（または）の例 ----------
let ateMeal = false;   // ご飯を食べたか（true = 食べた）
let ateSnack = true;   // お菓子を食べたか（true = 食べた）

// 「ご飯を食べた または お菓子を食べた ならお腹は満足」
if (ateMeal || ateSnack) { // どちらかが true なら実行
  console.log("🍽 ご飯かお菓子を食べたのでお腹は大丈夫！ 😋");
  console.log(` ↑↑↑ ateMeal = ${ateMeal} または ateSnack = ${ateSnack} のどちらかが true なのでOK。`);
} else {
  console.log("🍴 何も食べてないのでお腹が空いた… 🥺");
  console.log(` ← ateMeal = ${ateMeal} かつ ateSnack = ${ateSnack} が両方 false なのでNG。`);
}
