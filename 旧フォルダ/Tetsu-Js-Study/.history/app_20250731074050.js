...
console.log(
  "📢 呼び出し元 console。" +
  "最初の状態として、オブジェクト pikachu2 の初期レベルは 18。" +
  "スキル（skills）には 3 つの技が登録されている状態です。→",
  pikachu2.level,
  pikachu2.skills
);
// (2-1) 初期状態を表示。
//       → レベル: 18、スキル: ["10万ボルト", "でんこうせっか", "たいあたり"]

pikachu2.levelUp();
// (2-2) メソッド levelUp() を 1 回実行。
//       → this.level++ によりレベルが 19 に上昇。
//       → ただし、まだレベル 20 に達していないため「スパーク」は追加されない。

console.log(
  "📢 呼び出し元 console。" +
  "↑ 上の行で pikachu2.levelUp() を 1 回実行しました。" +
  "現在のレベルは 19。if 文の条件（レベル20以上）にはまだ届いていないため、" +
  "スキル『スパーク』はまだ追加されていません。→",
  pikachu2.level,
  pikachu2.skills
);
// (2-3) レベル 19 の状態を表示。
