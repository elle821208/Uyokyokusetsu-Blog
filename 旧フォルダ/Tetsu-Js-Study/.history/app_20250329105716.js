...
function hello(name) {//②関数hello。引数(name)に呼び出し元の値'サムライエンジニア太郎'が代入される。

    return 'こんにちは、' + name + 'さん';//③関数の実行内容=戻り値。引数の値を代入して計算する。

}

console.log(hello('サムライエンジニア太郎'));//①呼び出し元。引数の値に'サムライエンジニア太郎'を設定しておく。
//④呼び出す際に、戻り値'こんにちは、' + 'サムライエンジニア太郎' + 'さん'が代入される。
