...
const pikachu2 = {
  name: "ピカチュウ",
  level: 17,//初期値。下記levelUpメソッドのループで使用する。
  types: ["でんき"],
  skills: ["10万ボルト", "でんこうせっか", "たいあたり"],//配列skills。後で4番目の"スパーク"を追加する。powerUpメソッドのif文で判定する。
  
  levelUp: function () {
      for (let i = 0; i < 1; i++) {//真ん中の条件式(i < 1)のために変数iは1回だけループする。
      ...
      console.log(this.level); // pikachu2.level（オブジェクトのプロパティ）の現在の値を表示
      //※※もしconsole.logがない場合、画面には何も表示されません。
      // でも実はlevelは内部的にちゃんと 1 → 2 と増えているんです。
      // ただ、開発者から見えない（確認できない）だけなんです。
    },
  
    powerUp: function(){
        if (this.level >= 20) {//もしthis.level(pikachu2オブジェクトのプロパティlevel)の値が20以上の場合
            this.skills.push("スパーク");//プロパティskills配列の値にpushを追加する。
        }
        console.log(this.skills[3]);//this.skills（プロパティ配列skillsの4番目）を表示する。
    },
};

//インクレメント++はオブジェクトpikachu2の中で記憶され初期値levelを増やしていく。
//メソッドincrementを呼び出す度にthis.value見て、それを1だけ増やします。
pikachu2.levelUp();//18。levelUpメソッドを実行する。
pikachu2.levelUp();//19。同上
pikachu2.levelUp();//20。同上
pikachu2.powerUp();//20。powerUpメソッドを実行する。if文判定させて、配列に"スパーク"を追加させる。
...
//メソッドlevelUpを実行すると
  ...
  //if文（定数と変数を宣言。数字の大小で判定。変数の値を再代入する。）
//Q.「myMoneyが500以上ならisShippingでtrueを返す」<!--1:17:30～-->
const myMoney22 = 1000; //その１。定数を宣言: myMoneyの値は1000を代入する
let isShipping22 = false; //その２。変数を宣言: isShippingの値はfalseを代入しておく。※後で再代入するためにletにする。
//上記で定義した定数myMoneyと定数の値（今回は"1000"）を下記のif文で判定する。
if (myMoney22 >= 500) {
    //条件式: もし定数myMoneyが500以上なら
    //{}内の処理を行う。
...
console.log(isShipping22); //isShippingを出力する。
...
//メソッドを実行すると値が1ずつ増える
const counter = {
    //オブジェクトcounter。{}内はプロパティ情報。
    value: 0, // プロパティ名value。値は0。この値を下記のforループ文（incrementメソッド）の初期値0にする。
    //valueはオブジェクトcounterの中で「記憶」されています
  increment: function () {
    //メソッド名increment。{}内は処理。for文で繰り返す。変数iを設定。変数iの初期値:0、条件式:変数iが1より小さい時、増減式:変数iを1ずつ増やす。
    for (let i = 0; i < 1; i++) {// 変数i。真ん中の条件式の設定(i < 1)でループは1回だけ実行される。初期値:変数iは最初は0、条件式:変数iは1より小さい時、増減式:変数iを1ずつ増やす。
        this.value++; // counter.value（オブジェクトのプロパティ）の値を++インクレメントで1ずつ増やす。
        //インクレメント（++）は、オブジェクトのプロパティを変更する操作
        //一度変更されたプロパティは、次の閲覧でも前の結果を引き継ぐ
        //だから、実行するたびに1 → 2 → 3 → 4 …増えていくのです
    }
      console.log(this.value); // counter.value（オブジェクトのプロパティ）の現在の値を表示
      //※※もしconsole.logがない場合、画面には何も表示されません。
      // でも実はvalueは内部的にちゃんと 1 → 2 と増えているんです。
      // ただ、開発者から見えない（確認できない）だけなんです。
  },
};

//インクレメント++はオブジェクトcounterの中で記憶され初期値valueを増やしていく。
//メソッドincrementを呼び出す度にthis.value見て、それを1だけ増やします。
counter.increment(); // 値は1。オブジェクトcounterのincrementメソッド。値は1
counter.increment(); // 値は2
counter.increment(); // 値は3

// 🔹どうして毎回増えるのか？
// valueはオブジェクトcounterの中で「記憶」されています
// メソッドincrementを呼び出す度にthis.value見て、それを1だけ増やします。
// for文の中でそれを実行しているだけで、毎回前回の値に1を足していることになります。
...
//上記の配列をfor文で繰り返す。
for (let index = 0; index < questions2.length; index++) {
  //初期値:変数indexの最初の配列の数は0、条件式:question2の配列の数より小さい時、増減式:index配列の数を1ずつ増やしていく
  console.log(questions2[index]); //配列question2のindex数(上記で繰り返した数)だけ呼び出す。
}

///戻り値と関数 returnとundefind
///undefindがあらわる理由
