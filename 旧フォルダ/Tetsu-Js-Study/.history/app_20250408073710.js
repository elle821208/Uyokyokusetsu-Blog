...
///<!--2:08:03～-->
////appendChildで情報を持った変数を入れ込む。
//↓↓↓JSの変数$postにhtml用の情報（タグの種類、クラス、クラス名、文章"おなか減ったなう"）を入れる
    const $post = document.createElement("article");//変数$postを宣言する。createElementでHTMLタグを生成する。（今回は引数どおりのarticleタグを生成する。）
    $post.setAttribute("class", "post");//上記の変数$post"articleタグ"に"クラス"（第一引数）を付与し、"post"（第二引数）というクラス名をつける。
    $post.innerText = "お腹減ったなう。";//上記の変数$postに文章"おなか減ったなう"を付け足す。
//↓↓↓JSの変数$timelineでHTMLからtimelineクラス（この段階ではまだ情報は空っぽ）を取得し、上記の$postの情報を入れる。
    const $timeline = document.querySelectorAll(".timeline")[0];//変数timelineを宣言する。querySelectorAllでHTMLに記述した.timeline(引数)クラスの[]番目を取得する。
$timeline.appendChild($post);//上記の空っぽの変数$timelineにappendChildで$post(引数)を入れる。
...
window.querySelector("#button22").addEventListener("click", function () { 
