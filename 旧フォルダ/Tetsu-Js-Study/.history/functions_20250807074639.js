...
// 名前を表示する関数
function showName() {
    console.log(" ③追加関数showName。※← ここの()の引数=追加関数を複数設定して追加処理を増やすことができる。"
        + "追加処理 → Hi,Bob!。");
}

// ↓↓↓ 呼び出し元
// それぞれの関数をコールバックとして渡す
console.log("↓↓↓  ①呼び出し元greet(showName)。※← ここの()の引数=追加関数を複数設定して追加処理を増やすことができる。"
    + "下記で関数greet(引数showName)を実行する。上記のメイン処理greet(cb)を実行する。");
greet(showName);
